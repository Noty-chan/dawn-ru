...
const AUDIT_FINDINGS = new Map([
  ...
  ["vagabond.assassin.3", "Канонический Hide → Stride теперь проверяется ядром: бесплатный Шаг, Невидим и [Скорость/2] к следующему Завершению в этот Ход; происхождение последовательности и digest обязательны."],
  ...
  ["altruist.empath.3", "Каноническая passive boundary-ветка автоматически выдаёт соседнему союзнику 3 Фокуса и [Ступень] Здоровья в начале его Хода; прежний платный Support-путь удалён во избежание двойного применения."],
  ...
  ["altruist.alchemist.1", "Заявление full понижено до partial: Чистая вода удаляет все Эффекты без канонического выбора любого подмножества. Опциональный отказ от создания Зелья и запрет неканонических типов добавлены при повторном аудите."],
  ["altruist.surgeon.1", "Заявление full понижено до partial: core проверяет наличие массива rolls, но доверяет присланным successes и не доказывает обязательный бросок Разума."],
  ["altruist.chronomancer.2", "Реестр заявлял manual, хотя core уже выдаёт 1 Преимущество Заклинаниям и ведёт prompt повторного применения снятого Эффекта за 1 Фокус; добавлен честный decision-adapter. Полный surface evidence отсутствует."],
  ["altruist.will-o-wisp.1", "Заявление decision понижено до partial: первая Зарядка, аура и выбранное движение есть, но Пламя не толкается на 1 клетку, когда Атака выбирает целью его клетку."],
  ["altruist.will-o-wisp.2", "Заявление decision понижено до partial: остановка врага происходит после полного actor.move и ретроспективно возвращает его в первую клетку выхода, поэтому downstream enter/path-триггеры уже могли сработать за канонической точкой остановки."],
  ["disruptor.constrictor.2", "До повторного аудита дополнительный урон Ступени ошибочно требовал Пойман именно от этого Душителя, хотя ограничение владельца относится только к дистанционному targeting; исправлено для любого Пойманного персонажа с отдельным regression."],
  ["disruptor.chemist.3", "До повторного аудита Осаждение срабатывало только для зон с source/ruleId Сублимации, хотя канон говорит о любой создаваемой владельцем зоне Газа; trigger расширен по areaType=gas и закреплён regression с другим источником."],
  ["disruptor.chemist.2", "Здоровье цели читается и KO применяется автоматически вместо вопроса Нарратору."],
  ["disruptor.autophage.1", "Заявление full понижено до partial: замена Фокуса работает, но Регенерация требует `expectedDamage > 0`, тогда как канон требует успешной Атаки и не отменяется полной Бронёй/Уворотом."],
  ["disruptor.autophage.2", "Заявление decision понижено до partial: preview ищет любой прежний успешный удар в журнале Сцены и не доказывает незавершённое окно текущей Атаки; возможна поздняя повторная активация."],
  ["disruptor.autophage.3", "Заявление decision понижено до partial: once/Scene и выбор всех врагов с большим HP есть, но запуск наследует stale-hit уровня II и не атомарен с текущим Завершением."],
  ["disruptor.mind-breaker.2", "Реестр заявлял manual, хотя source-aware Изгнание уже производит Помечен для врага и Усилен для союзника. Добавлен partial-adapter; уровень I и полный пользовательский путь отсутствуют."],
  ["disruptor.mind-breaker.3", "Реестр заявлял manual, хотя единственная source-owned Изгнанная цель уже сохраняет эффект в начале Хода. Добавлен partial-adapter; end-Turn перемещение и принудительная Атака отсутствуют."],
  ["disruptor.reaper.2", "Реестр заявлял manual, хотя effect lifecycle уже удерживает Помечен на враге в пределах 3 клеток. Добавлен partial-adapter; источник Метки уровнем I и surface evidence отсутствуют."],
  ["disruptor.inner-world.1", "Заявление decision понижено до partial: после action.resolve Зарядки trigger ищет последнее подходящее получение Фокуса во всём журнале и может повторно использовать старое событие, если текущая Зарядка дала 0 или ресурс был заменён."],
  ["ruiner.bombardier.1", "Заявление full понижено до partial: авторитетный план требует Завершение Духом, вражескую цель в центре и дальность 4, затем выводит центр и смежные клетки; полный сетевой/UI surface-контракт ещё не сертифицирован."],
  ["ruiner.bombardier.2", "Заявление full понижено до partial: авторитетно проверяются Завершение Духом, Focus≥2, зона 3×3 в дальности 5, производный счёт пустых клеток и предел [Ступень+2]; полный surface-контракт остаётся ручным."],
  ["ruiner.bombardier.3", "Заявление full понижено до partial: авторитетно проверяются Завершение Духом, Focus≥4 и зона 5×5 в дальности 6; полный surface-контракт остаётся ручным."],
  ["disruptor.siren.1", "Старый текстовый путь ошибочно обещал ограничение «3 раза за Сцену». Английский канон требует +3 Фокуса в начале каждой Сцены без этого лимита; авторитетный Study→Fear choice-flow и digest добавлены, cross-surface certification остаётся ручной."],
  ["disruptor.siren.2", "Сирена II требует once-per-Turn receipt после собственного Fear-события; авторитетно проверены владелец, digest, живой враг, пошаговое движение до 3 и Daze/+1 Focus только после фактической смежности. UI/network/import surface evidence остаётся ручным."],
  ["ruiner.spellcrafter.1", "Решение выбора сохраняется в листе, сцене и импорте; уровень изучает одну Модификацию, а перед применением можно выбрать только изученные варианты."],
  ["ruiner.spellcrafter.2", "Решение выбора сохраняется в листе, сцене и импорте; этот уровень добавляет вторую изученную Модификацию, отключает Новаторство и переводит оплату на Фокус."],
  ["ruiner.spellcrafter.3", "Решение выбора сохраняется в листе, сцене и импорте; этот уровень добавляет третью изученную Модификацию, а одно действие всё ещё может применить не больше двух разных вариантов."],
  ["ruiner.ritualist.1", "Заявление decision понижено до partial: круг создаётся отдельной кнопкой без обязательной Зарядки и без отказа от полученного ею Фокуса; работают лишь placement/replacement и бонус лимита Spirit Finisher."],
  ["ruiner.cryomancer.2", "Заявление decision понижено до partial: старый prompt использовал любой текущий Focus; новый adapter/core требует фактический Focus gain с action provenance, ведёт 4-сегментную Сосульку и каноническую замену Breathe, но полного UI/network/save evidence ещё нет."],
  ["ruiner.grim-ascendant.2", "Старый state-toggle с половиной урона и Регенерацией не соответствует новому EN-канону. Реестр понижен до partial; каноническая ветка Drain Life ведётся отдельным связным trigger-путём и требует полного surface evidence."],
