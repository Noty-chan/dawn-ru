...
function lwOperationSummary(p){
  const names={attack:"Атака",damage:"Урон","spend-health":"Расход Здоровья","lose-health":"Потеря Здоровья",heal:"Лечение",effect:p.remove?"Снять Эффект":"Наложить Эффект",resource:p.operation==="spend"?"Расход":"Получение",move:"Движение",wound:"Рана",stress:"Стресс",modifier:"Модификатор",usage:"Учесть применение","record-action":"Базовое действие","recover-track":"Восстановление Ран/Стресса",prompt:"Решение",note:"Запись", "grant-turn":"Дополнительный Ход","allow-action":"Допуск действия"};
...
document.addEventListener("click", event => {
  ...
  if (button.hasAttribute("data-lw-operation")) {
    ...
    if(kind==="attack"){
      ...
      return lwSubmit(actorId,costs.length?{kind:"plan",costs,operations:[attack],targetIds:targets,actionId:"manual.attack"}:attack,"Ручная Атака");
    }
    const operations=[];
