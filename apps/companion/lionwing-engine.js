...
(function installLionwingEngine(global) {
  ...
  function prepare(scene, request, options = {}) {
    try {
      ...
      const payload = copy(request), a = (["scene-reset","round-end","intermission","tension","note","banish","vanish","compound"].includes(payload.kind)&&!payload.actorId || payload.kind === "inventory" && payload.operation === "reset" && !payload.actorId) ? null : requiredActor(scene, payload.actorId, !["choice", "correct", "resolve-attack", "cancel-attack", "batch","banish","vanish","compound"].includes(payload.kind));
