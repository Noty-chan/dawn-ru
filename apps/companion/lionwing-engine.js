...
(function installLionwingEngine(global) {
  ...
  function execute(scene, event, output, executionOptions = {}) {
    ...
    const emit = (type, actorId, payload = {}) => {
      ...
      if (scheduleAfterEvent && ["clash.success", "damage.apply", "actor.knockout", "effect.apply", "actor.enter", "action.resolve", "marker.remove", "resource.gain"].includes(type)) scheduleAfterEvent(row);
      return row;
    };
