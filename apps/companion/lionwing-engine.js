...
(function installLionwingEngine(global) {
  ...
  function dispatchMany(scene, events, options = {}) {
    ...
    const modifierFlow = events.length > 0 && events.every(event => event?.type === "modifier.configure" && (scene.actors || []).some(actor => actor.id === event.actorId && ["lionwing.modifier.isolation","lionwing.modifier.artillery","lionwing.modifier.haven","lionwing.modifier.contagion","lionwing.modifier.earthquake","lionwing.modifier.vip","lionwing.modifier.collateral","lionwing.modifier.legion"].includes(actor.profileId)) || event?.type === "rule.respond" && ["modifier-refresh","lionwing-vip-follow"].includes(scene.pendingPrompt?.kind) && scene.pendingPrompt?.sourceActorId === event.actorId && (scene.actors || []).some(actor => actor.id === event.actorId && ["lionwing.modifier.isolation","lionwing.modifier.artillery","lionwing.modifier.haven","lionwing.modifier.contagion","lionwing.modifier.earthquake","lionwing.modifier.vip","lionwing.modifier.collateral","lionwing.modifier.legion"].includes(actor.profileId)) || event?.type === "actor.move" && event.payload?.vipFollow && (scene.actors || []).some(actor => actor.id === event.actorId && actor.profileId === "lionwing.modifier.vip"));
    ...
    for (const raw of events) {
      ...
      for (const knockout of output.slice(outputStart).filter(item => item.type === "actor.knockout")) {
        ...
        if (!["lionwing.modifier.vip","lionwing.modifier.collateral"].includes(defeated?.profileId) && !(defeated?.team === "enemy" && defeated?.kind !== "crowd" && (next.actors || []).some(item => item.profileId === "lionwing.modifier.legion" && item.modifierState?.deployed))) continue;
