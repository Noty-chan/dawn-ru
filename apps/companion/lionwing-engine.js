...
(function installLionwingEngine(global) {
  ...
  function dispatchMany(scene, events, options = {}) {
    ...
    const modifierFlow = events.length > 0 && events.every(event => event?.type === "modifier.configure" && (scene.actors || []).some(actor => actor.id === event.actorId && ["lionwing.modifier.isolation","lionwing.modifier.artillery","lionwing.modifier.haven","lionwing.modifier.contagion","lionwing.modifier.earthquake","lionwing.modifier.vip","lionwing.modifier.collateral","lionwing.modifier.legion","lionwing.modifier.vortex","lionwing.modifier.blaze","lionwing.modifier.giant"].includes(actor.profileId)) || event?.type === "rule.respond" && ["modifier-refresh","lionwing-vip-follow"].includes(scene.pendingPrompt?.kind) && scene.pendingPrompt?.sourceActorId === event.actorId && (scene.actors || []).some(actor => actor.id === event.actorId && ["lionwing.modifier.isolation","lionwing.modifier.artillery","lionwing.modifier.haven","lionwing.modifier.contagion","lionwing.modifier.earthquake","lionwing.modifier.vip","lionwing.modifier.collateral","lionwing.modifier.legion","lionwing.modifier.vortex","lionwing.modifier.blaze","lionwing.modifier.giant"].includes(actor.profileId)) || event?.type === "actor.move" && event.payload?.vipFollow && (scene.actors || []).some(actor => actor.id === event.actorId && actor.profileId === "lionwing.modifier.vip"));
    if (modifierFlow) return legacy.dispatchMany(scene, events, options);
    const giantAction=events.find(event=>event?.type==="modifier.action"&&(scene.actors||[]).some(item=>item.id===event.actorId&&item.profileId==="lionwing.modifier.giant"));
    if(giantAction){
      const giant=(scene.actors||[]).find(item=>item.id===giantAction.actorId),host=(scene.actors||[]).find(item=>item.id===giant?.modifierState?.carrierId),spends=events.filter(event=>event?.type==="resource.spend"&&event.actorId===host?.id&&event.payload?.resource==="ap"&&event.payload?.amount===1&&event.payload?.sourceActionId==="lionwing.modifier.giant.attack");
      if(!host||events.length!==2||events[0]!==giantAction||spends.length!==1)fail("Атака Гиганта требует ровно 1 ОД носителя");
