...
function renderSavedCampaigns(){
  const select=$("sync-table-select"),selected=select.value,roleNames={owner:"Нарратор",narrator:"Нарратор",player:"Игрок"};select.innerHTML=savedCampaigns.length?`<option value="">Выберите стол</option>${savedCampaigns.map(campaign=>`<option value="${campaign.sceneId}">${esc(campaign.name)} · ${roleNames[campaign.role]||campaign.role} · версия ${campaign.version}</option>`).join("")}`:`<option value="">Сохранённых столов пока нет</option>`;if(savedCampaigns.some(campaign=>campaign.sceneId===selected))select.value=selected;$("sync-table-open").disabled=!select.value;
}
