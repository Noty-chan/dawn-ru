...
function resetClientTableRuntime(){
  resetNetworkV2Runtime();pendingSceneCommands=[];pendingCommandSceneId=null;automaticCommandAttempts.clear();delayedAutomaticCommands.clear();lastInviteToken="";
...
function queueAutomaticCommand(command){
  ...
  automaticCommandChain=automaticCommandChain.catch(()=>{}).then(async()=>{
    ...
    if(!current)return;
    try{const wasDelayed=delayedAutomaticCommands.has(id);await decideSceneCommand(current,"applied");if(wasDelayed)toast("Задержанное действие игрока обработано")}
    catch(error){automaticCommandAttempts.delete(id);delayedAutomaticCommands.add(id);renderSync();toast(`Действие игрока задержано: ${friendlySyncError(error)}`);setTimeout(()=>{const retry=pendingSceneCommands.find(item=>String(item.id)===id);if(retry)queueAutomaticCommand(retry)},1200)}
  });
}
