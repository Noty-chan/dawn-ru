...
Sync?.on("scene",payload=>{if(!payload?.state||typeof payload.state!=="object")return;const previousRequestId=Scene.challengeRequest?.id||"",sceneId=Sync.state().sceneId;if(sceneId!==pendingCommandSceneId){pendingSceneCommands=[];automaticCommandAttempts.clear();pendingCommandSceneId=sceneId}const remote=mergeNetworkV2Scene({...payload.state,version:Number(payload.version??payload.state.version??0)},Scene);Scene=hydratePlayerScene(remote);store.scene=Scene;persist();if(store.mode==="play")renderPlay();if(store.mode==="tools")renderToolsWorkspace();renderChallengeRequestDock();const ownRequest=currentChallengeRequest();if(ownRequest&&ownRequest.id!==previousRequestId)toast(`Нарратор запросил бросок · цель ${ownRequest.target}`);queueAutomaticCommands()});
