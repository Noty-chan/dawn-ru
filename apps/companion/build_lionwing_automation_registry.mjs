...
const runtimeTarget = path.join(root, "lionwing-automation-status.js");
...
// The JSON registry remains the source of truth. This intentionally small
// generated projection is the only registry data shipped to the browser: it
// carries the per-level status and reviewed partial-coverage note without
// duplicating canonical technique text or accepting client payload fields.
function buildRuntimeStatus(registry) {
  const rows = registry.rows.map(row => {
    const notes = (row.implementation?.rules || [])
      .map(rule => String(rule.note || "").replace(/\s+/g, " ").trim())
      .filter(Boolean);
    return {
      id: row.id,
      automation: ["full", "decision", "partial", "manual"].includes(row.implementation?.automation)
        ? row.implementation.automation
        : "manual",
      reason: [...new Set(notes)].join(" "),
      canonicalDigest: row.provenance?.canonicalDigest || null,
    };
  });
  return {
    schemaVersion: 1,
    editionId: registry.editionId,
    canonical: clone(registry.canonical),
    rows,
  };
}

function buildRuntimeScript(registry) {
  return `// Generated by build_lionwing_automation_registry.mjs from LIONWING-AUTOMATION-REGISTRY.json.\nwindow.DAWN_LIONWING_AUTOMATION_STATUS = ${JSON.stringify(buildRuntimeStatus(registry), null, 2)};\n`;
}
...
const outputRuntime = buildRuntimeScript(registry);
if (process.argv.includes("--check")) {
  ...
  const actualRuntime = fs.existsSync(runtimeTarget) ? fs.readFileSync(runtimeTarget, "utf8").replace(/\r\n/g, "\n") : "";
  if (actualJson !== outputJson || actualMarkdown !== outputMarkdown || actualRuntime !== outputRuntime) {
    console.error("LionWing automation registry/runtime status is stale. Run npm run build:lionwing-registry.");
    process.exitCode = 1;
  } else {
  ...
  fs.writeFileSync(runtimeTarget, outputRuntime, "utf8");
  console.log(`LionWing automation registry written: ${registry.rows.length} rows, ${registry.rows.filter(row => row.review.explicit).length} explicit reviews, browser status projection updated`);
}

export { buildMarkdown, buildRegistry, buildRuntimeScript, buildRuntimeStatus, canonicalLevelDigest, canonicalRows, validateEvidence, validateReview };
