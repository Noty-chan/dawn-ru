...
const LIONWING_AUTO_ATTACK_RULES = new Map([
  ...
  ["lionwing.npc.swarm.tear", { dice: "5(+1)", tensionMultiplier: 1, targetEffects: [], reward: "Deal [Hits] + [Tension] damage and Daze one target.", family: { maxTargets: 3, crowdAdvance: 1, targetAdjacentToCrowd: true, chooseOneEffect: "Ошеломлен" } }],
...
const ENEMY_FULL_RULES = new Map([
  ...
  ["lionwing.npc.pugilist.martial-perfection", { type: "martial-perfection" }],
...
function prepareAction(scene, data, request = {}) {
  ...
  if (armamentQuick) {
    ...
    events.push({ type: "rule-mode.set", actorId: actor.id, payload: { groupId: MASTER_AT_ARMS_GROUP, modeId: armamentMode, actionInstanceId, ruleId: armament.contract?.sourceRuleId || "vagabond.master-at-arms.1", sourceRuleId: armament.contract?.sourceRuleId || "vagabond.master-at-arms.1", sourceDigest: armament.contract?.sourceDigest || null, actionId: armament.contract?.actionId || declaredAction.id, receipt: armament.contract?.receipt || null, participantIds: [actor.id, ...targetIds] } });
  }
  ...
  const limit = (armamentMode === "chain" || gunslingerSkirmish || knifeThrow || breacherSkirmish ? 4 : actionIs(action, "spell") ? 5 : 1) + (spellModifiers.includes("outstanding") ? Number(actor.attrs?.mind || 0) : 0);
    ...
    const bonus = (actionIs(action, "finish") ? Number(scene.tension || 0) : 0) + (spellModifiers.includes("fierce") ? Number(actor.attrs?.mind || 0) : 0), adjustedDamage = value => Math.max(0, value);
    if (heavenlyHealing) {
      ...
      const effectDamageDivisor = eclipseStars ? 2 : 1;
