...
(function exposeDawnNetworkV2(global){
  ...
  function materializeIntent(scene,data,intent,ownerId,{sceneEngine,techniqueEngine,safeTechniqueEffects}={}){
    ...
    if(intent.kind==="lionwing"){
      ...
      if(!kernel?.isScene(scene)||!["action","plan","reaction","choice","roll","punish","invisible","search"].includes(raw.kind))throw new Error("Эта операция LionWing доступна только Нарратору");
