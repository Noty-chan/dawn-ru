...
const wordById=id=>Object.values(D.abilityWords).flat().find(w=>w.id===id);
...
function renderOutlooks(){
  $("outlooks").innerHTML=D.outlooks.map(o=>{const selected=S.outlooks.includes(o.id),primary=S.primaryOutlook===o.id;return `<button type="button" class="choice ${selected?"selected":""} ${primary?"primary-choice":""}" data-outlook="${o.id}" aria-pressed="${selected}"><strong>${esc(o.name)}</strong><small>${selected?(primary?"Нажмите, чтобы убрать":"Нажмите, чтобы сделать Основным"):esc(o.category)}</small></button>`}).join("");
...
$("outlooks").addEventListener("click",event=>{const b=event.target.closest("[data-outlook]");if(!b)return;const id=b.dataset.outlook;if(!S.outlooks.includes(id)){const maximum=Math.min(3,S.tier);if(S.outlooks.length>=maximum)return toast(`На ${S.tier}-й Ступени доступно Мировоззрений: ${maximum}`);S.outlooks.push(id);if(!S.primaryOutlook)S.primaryOutlook=id;}else if(S.primaryOutlook!==id){S.primaryOutlook=id;}else{S.outlooks=S.outlooks.filter(x=>x!==id);S.primaryOutlook=S.outlooks[0]||null;const allowed=new Set(D.outlooks.filter(o=>S.outlooks.includes(o.id)).flatMap(o=>o.gifts.map(g=>g.id)));S.gifts=S.gifts.filter(g=>allowed.has(g));}renderAll();});
...
$("ability-builder").addEventListener("click",event=>{const add=event.target.dataset.wordAdd;if(add){const id=$("word-"+add).value;if(id&&!S.ability.words[add].includes(id))S.ability.words[add].push(id);renderAll();return;}const remove=event.target.dataset.wordRemove,group=event.target.dataset.wordGroup;if(remove&&group){S.ability.words[group]=S.ability.words[group].filter(id=>id!==remove);renderAll();}});
...
$("ability-builder").addEventListener("change",event=>{if(event.target.id==="ability-rank")S.ability.rank=+event.target.value;renderAll();});
