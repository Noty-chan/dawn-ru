...
function respondRulePrompt(scene, data, request = {}) {
  ...
  if (prompt.kind === "chemist-sublimation" && choice === "sublimate") {
    ...
    if (!terrain) return { ok: false, errors: ["Выбранная местность уже уничтожена."], events: [] };
    events.push({ type: "area.remove", actorId: actor.id, payload: { id: terrain.id, label: terrain.label, sourceActionId: "disruptor.chemist.1" } });
    events.push({ type: "area.create", actorId: actor.id, payload: { id: `gas-${prompt.id}`, space: terrain.space, areaType: "gas", label: "Сублимация", source: "disruptor.chemist.1", ruleId: "disruptor.chemist.1", duration: "nextTurn", ownerActorId: actor.id, cells: clone(prompt.context?.cells || []), participantIds: [actor.id] } });
  }
