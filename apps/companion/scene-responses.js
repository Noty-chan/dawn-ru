...
function ruleChoiceStatus(scene, request = {}) {
  ...
  else if (!source || source.knockedOut && !prompt.context?.revenantReturn) reason = "Источник решения больше не доступен.";
...
function respondRulePrompt(scene, data, request = {}) {
  ...
  if (prompt.kind === "enemy-coordinator-followup-ally" && choice.startsWith("ally:")) {
    ...
    const family = primary ? enemyAttackFamilyForRule(primary.id) : {};
  ...
  if (prompt.kind === "enemy-cocoon-menace-ally" && choice.startsWith("ally:")) {
    const ally=actorById(scene,choice.slice(5));
    if(!ally||ally.knockedOut||ally.id===actor.id||ally.team!==actor.team||!effectPresenceStatus(scene,ally.id).onField)return{ok:false,errors:["Выбранный союзник Кокона больше недоступен."],events:[]};
    const primary=enemyProfileById(data,ally.profileId)?.rules?.find(rule=>rule.kind==="attack"),family=primary?enemyAttackFamilyForRule(primary.id):{};
    const candidates=primary?(scene.actors||[]).filter(candidate=>{
      if(candidate.knockedOut||candidate.team===ally.team||candidate.space!==ally.space||!effectTargetingStatus(scene,ally.id,candidate.id).available||!wallTargetingStatus(scene,ally,candidate,{range:primary.range}).available)return false;
      if((primary.adjacent||family.adjacent)&&distance(ally,candidate)>1)return false;
      if(primary.range&&distance(ally,candidate)>Number(primary.range))return false;
      return true;
    }):[];
    const options=["move",...candidates.map(candidate=>`attack:${candidate.id}`),"pass"],optionLabels={move:`Переместить на Скорость (${Number(ally.speed||0)})`,pass:"Не использовать"};
    for(const candidate of candidates)optionLabels[`attack:${candidate.id}`]=`${primary.name}: ${candidate.name}`;
    events.push({type:"rule.prompt",actorId:ally.id,payload:{id:`prompt-${prompt.id}-${ally.id}`,kind:"enemy-coordinator-followup-action",sourceActorId:ally.id,controller:"narrator",title:"Угроза: действие союзника",text:`${ally.name} может переместиться на свою Скорость или бесплатно использовать основную Атаку.`,options,context:{coordinatorId:actor.id,cocoonMenace:true,ruleId:primary?.id||null,eligibleTargetIds:candidates.map(candidate=>candidate.id),maxDistance:Number(ally.speed||0),optionLabels},participantIds:[actor.id,ally.id,...candidates.map(candidate=>candidate.id)]}});
  }
  if (prompt.kind === "enemy-coordinator-followup-action") {
    const cocoonMenace=Boolean(prompt.context?.cocoonMenace),followupSource=cocoonMenace?"lionwing.npc.cocoon.menace":"enemy.common.coordinator.attack.fanaticize",followupName=cocoonMenace?"Угроза":"Фанатизировать";
    if (choice === "move") {
      ...
      events.push({ type: "rule.prompt", actorId: actor.id, payload: { id: `prompt-${prompt.id}-move`, kind: "enemy-move-cell", sourceActorId: actor.id, controller: "narrator", title: `${followupName}: перемещение союзника`, text: `Переместите ${actor.name} на расстояние до ${movement.distance} клеток.`, options: ["cancel"], context: { maxDistance: movement.distance, coordinatorFollowup: true, sourceActionId:followupSource }, participantIds: [actor.id, prompt.context?.coordinatorId].filter(Boolean) } });
    } else if (choice.startsWith("attack:")) {
      const targetId = choice.slice(7), target = actorById(scene, targetId), coordinator = actorById(scene, prompt.context?.coordinatorId), rule = enemyCanonicalRule(enemyProfileById(data, actor.profileId)?.rules?.find(item => item.id === prompt.context?.ruleId)), family = rule ? enemyAttackFamilyForRule(rule.id) : {};
      ...
      if (!target || target.knockedOut || target.team === actor.team || !cocoonMenace&&attacked.has(target.id) || !(prompt.context?.eligibleTargetIds || []).includes(target.id) || !rule || !effectTargetingStatus(scene, actor.id, target.id).available || !wallTargetingStatus(scene, actor, target, { range: rule.range }).available) return { ok: false, errors: ["Цель дополнительной Атаки больше недоступна или уже была атакована в этот Ход."], events: [] };
      ...
      events.push({ type: "enemy.action.prepare", actorId: actor.id, payload: { ruleId: rule.id, sourceRuleId: rule.sourceRuleId || rule.id, sourceDigest: rule.sourceDigest || null, profileId: actor.profileId, name: `${rule.name} · ${followupName}`, kind: "attack", targetIds: [target.id], automation: "attack", quickReaction: true, free: true, sourceActionId: followupSource, participantIds: [actor.id, target.id, coordinator?.id].filter(Boolean) } });
      ...
      events.push({ type: "attack.pending", actorId: actor.id, payload: { actionId: rule.id, enemyRuleId: rule.id, sourceRuleId: rule.sourceRuleId || rule.id, sourceDigest: rule.sourceDigest || null, name: `${rule.name} · ${followupName}`, targetIds: [target.id], roll: expectedDice ? clone(roll) : null, damage, damageByTarget: { [target.id]: damage }, effects: targetEffects, reward: rule.reward || "", enemyAttackFamily: clone(family), quickReaction: true, sourceActionId: followupSource, participantIds: [actor.id, target.id, coordinator?.id].filter(Boolean) } });
    }
  }
  ...
  if (prompt.kind === "cryomancer-icicle-rest" && choice === "convert") {
    ...
    const targets = (scene.actors || []).filter(candidate => candidate.id !== actor.id && !candidate.knockedOut && candidate.team !== actor.team && candidate.space === actor.space && distance(actor, candidate) <= 5);
    if (segments < 1 || !exactBreatheGain || alreadyConverted || Number(actor.focus || 0) < Number(gain?.payload?.resolvedDelta || 0) || !targets.length) return { ok: false, errors: [!targets.length ? "Для Сосульки больше нет допустимой цели в пределах 5 клеток." : "Сосулька или Фокус именно этой Передышки уже недоступны."], events: [] };
    events.push({ type: "rule.prompt", actorId: actor.id, payload: { id: `prompt-${prompt.id}-target`, kind: "cryomancer-icicle-target", sourceActorId: actor.id, title: "Ледяной нимб · цель", text: "Выберите персонажа в пределах 5 клеток для урона Дух/2 за каждый сегмент.", options: targets.map(candidate => `target:${candidate.id}`), context: { actionInstanceId: prompt.context.actionInstanceId, focusGainEventId: gain.id, segments, optionLabels: Object.fromEntries(targets.map(candidate => [`target:${candidate.id}`, candidate.name])) }, participantIds: [actor.id, ...targets.map(candidate => candidate.id)] } });
  }
  if (prompt.kind === "cryomancer-icicle-target" && choice.startsWith("target:")) {
    const selected = actorById(scene, choice.slice(7)), gain = (scene.log || []).find(event => event.id === prompt.context?.focusGainEventId), segments = Number(prompt.context?.segments || 0), clock = clockStatus(scene, actor.id, "ruiner.cryomancer.icicle"), alreadyConverted = (scene.log || []).some(event => event.type === "resource.spend" && event.payload?.forgoneGainEventId === gain?.id), exactBreatheGain = gain?.type === "resource.gain" && gain.actorId === actor.id && gain.payload?.actionInstanceId === prompt.context?.actionInstanceId && actionIdIs(gain.payload?.sourceActionId, "breathe") && gain.payload?.resolvedResource === "focus" && Number(gain.payload?.resolvedDelta || 0) > 0;
    if (!selected || selected.knockedOut || selected.team === actor.team || selected.space !== actor.space || distance(actor, selected) > 5 || !exactBreatheGain || alreadyConverted || clock.value < segments || Number(actor.focus || 0) < Number(gain?.payload?.resolvedDelta || 0) || segments < 1) return { ok: false, errors: ["Цель или условия Сосульки больше недоступны."], events: [] };
    const amount = Math.ceil(Number(actor.attrs?.spirit || 0) / 2) * segments;
    events.push({ type: "resource.spend", actorId: actor.id, payload: { actionInstanceId: prompt.context.actionInstanceId, forgoneGainEventId: gain.id, resource: "focus", amount: Number(gain.payload.resolvedDelta), sourceActionId: "ruiner.cryomancer.2", reason: "Отказ от Фокуса Передышки", participantIds: [actor.id, selected.id] } });
    events.push({ type: "rule-clock.set", actorId: actor.id, payload: { clockId: "ruiner.cryomancer.icicle", value: 0, sourceActionId: "ruiner.cryomancer.2", participantIds: [actor.id, selected.id] } });
    if (hasEffect(scene, selected, "negative.замедлен")) events.push({ type: "effect.apply", actorId: actor.id, payload: { targetId: selected.id, effect: "negative.обездвижен", sourceActionId: "ruiner.cryomancer.2", participantIds: [actor.id, selected.id] } });
    if (amount > 0) events.push({ type: "damage.apply", actorId: actor.id, payload: { targetId: selected.id, amount, sourceActionId: "ruiner.cryomancer.2", participantIds: [actor.id, selected.id] } });
    events.push({ type: "technique.resolve", actorId: actor.id, payload: { ruleId: "ruiner.cryomancer.2", name: "Ледяной нимб", affectedActorIds: [selected.id], segments, amount, participantIds: [actor.id, selected.id] } });
  }
...
function preparePromptPlacement(scene, request = {}) {
  ...
  if (prompt?.kind === "reappear-cell" && !prompt.context?.revenantReturn && (scene.actors || []).some(item => item.id !== actor.id && effectPresenceStatus(scene, item.id).onField && item.space === actor.space && distance(item, { ...destination, space: actor.space }) <= 1)) errors.push("При появлении клетка не должна быть смежна с персонажем.");
  ...
  if (["enemy-move-cell", "enemy-crowd-move-cell", "fodder-move-cell"].includes(prompt?.kind)) {
    ...
    const enemyTeleport = Boolean(prompt.kind === "enemy-move-cell" && prompt.context?.teleportFarthestAdjacent);
    enemyMovePath = unchanged ? [] : enemyTeleport ? [{ ...destination }] : prompt.kind === "fodder-move-cell" ? (fodderMoveDestinations(scene, mover.id).find(item => item.x === destination.x && item.y === destination.y)?.path || []).map(cell => { const [x,y]=cell.split(",").map(Number);return{x,y} }) : crowdPrompt && distance(mover, { ...destination, space: mover.space }) <= maximum ? [{ ...destination }] : movementPath(scene, mover.id, destination, { maxDistance: maximum, forced });
    if (!unchanged && !enemyMovePath.length) errors.push(`Выберите достижимую клетку в пределах ${maximum}.`);
    if (enemyTeleport) {
      const anchor = target, candidates = anchor ? [[1,0],[-1,0],[0,1],[0,-1]].map(([dx,dy])=>({x:anchor.x+dx,y:anchor.y+dy})).filter(point=>point.x>=0&&point.y>=0&&point.x<Number(space?.width||0)&&point.y<Number(space?.height||0)&&!removedCellKeys(scene,space.id).has(cellKey(point))&&effectCellOccupancyStatus(scene,mover.id,{space:space.id,...point}).available) : [];
      const farthest = candidates.length ? Math.max(...candidates.map(point=>distance(mover,{...point,space:mover.space}))) : -1;
      if (!anchor || distance(anchor,{...destination,space:anchor?.space}) !== 1 || distance(mover,{...destination,space:mover.space}) !== farthest) errors.push("Телепортация должна закончиться в самой дальней доступной клетке, смежной с целью.");
    }
    if (prompt.kind === "enemy-move-cell" && Number(prompt.context?.exactDistance || 0) > 0 && enemyMovePath.length !== Number(prompt.context.exactDistance)) errors.push(`Это перемещение должно пройти ровно ${prompt.context.exactDistance} клеток.`);
    if (prompt.kind === "enemy-move-cell" && prompt.context?.privateerEscort) {
      const escorted = actorById(scene, prompt.context.escortedActorId), source = escorted && effectStateFor(escorted, "positive.ускорен")?.sources.find(item => item.actorId === actor.id && item.actionId === "enemy.common.privateer.action.escort");
      if (!escorted || escorted.knockedOut || escorted.space !== actor.space || !source) errors.push("Связь Эскорта с союзником больше не действует.");
      else if (distance({ ...destination, space: actor.space }, escorted) > Number(prompt.context.maximumFinalDistance)) errors.push("Капер не может закончить дальше от сопровождаемого союзника.");
    }
  }
  let wavePath = [];
  ...
  if (prompt.kind === "wisp-create-cell") {
    ...
    if (prompt.kind === "reappear-cell" && prompt.context?.revenantReturn) events.push({type:"actor.knockout",actorId:actor.id,payload:{targetId:actor.id,restore:true,amount:Number(actor.maxHp||1),sourceActionId:"lionwing.npc.revenant.passive",participantIds:[actor.id]}});
    else if (prompt.kind === "reappear-cell") events.push({ type: "effect.remove", actorId: actor.id, payload: { targetId: actor.id, effect: "positive.исчез", sourceActionId: "reappear", participantIds: [actor.id] } });
  }
  return { ok: true, errors: [], events };
}
...
function resolvePendingAction(scene, data) {
  ...
  for (const targetId of status.eligibleIds) {
    ...
    if (!outcome.cancelled) {
      ...
      if (expectedDamage > 0 && pending.postResourceLoss?.resource && Number(pending.postResourceLoss.amount) > 0) {
        ...
        if (amount > 0) events.push({ type: "resource.spend", actorId: resolvedTargetId, payload: { resource: pending.postResourceLoss.resource, amount, sourceActionId: pending.postResourceLoss.ruleId || pending.enemyRuleId || pending.actionId, sourceRuleId: pending.postResourceLoss.ruleId || pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.postResourceLoss.sourceDigest || pending.sourceDigest || null, participantIds: [pending.actorId, resolvedTargetId] } });
      }
      if (expectedDamage > 0 && Number(pending.postSelfHealMissingFraction) > 0) postSelfHeal = true;
      if (expectedDamage > 0) for (const effect of pending.effects || []) events.push({ type: "effect.apply", actorId: pending.actorId, payload: { targetId: resolvedTargetId, effect, sourceActionId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, participantIds: [pending.actorId, resolvedTargetId] } });
      ...
      if (expectedDamage > 0 && enemyFamily.conditionalEffectsIfUntouched && untouchedThisRound) for (const effect of enemyFamily.conditionalEffectsIfUntouched) events.push({ type: "effect.apply", actorId: pending.actorId, payload: { targetId: resolvedTargetId, effect: effectIdByName(data, effect), sourceActionId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, participantIds: [pending.actorId, resolvedTargetId] } });
      if (expectedDamage > 0 && enemyFamily.enemyEffectsIfUntouched && target.team !== source?.team && untouchedThisRound) for (const effect of enemyFamily.enemyEffectsIfUntouched) events.push({ type: "effect.apply", actorId: pending.actorId, payload: { targetId: resolvedTargetId, effect: effectIdByName(data, effect), sourceActionId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, participantIds: [pending.actorId, resolvedTargetId] } });
      if (expectedDamage > 0 && enemyFamily.conditionalExistingEffect?.ifPresent && (target.effects || []).includes(effectIdByName(data, enemyFamily.conditionalExistingEffect.ifPresent))) events.push({ type: "effect.apply", actorId: pending.actorId, payload: { targetId: resolvedTargetId, effect: effectIdByName(data, enemyFamily.conditionalExistingEffect.apply), sourceActionId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, participantIds: [pending.actorId, resolvedTargetId] } });
      if (expectedDamage > 0 && enemyFamily.conditionalEffectsOnCrits && Number(pending.roll?.crits || 0) >= Number(enemyFamily.conditionalEffectsOnCrits.minimum || 0)) for (const effect of enemyFamily.conditionalEffectsOnCrits.effects || []) events.push({ type: "effect.apply", actorId: pending.actorId, payload: { targetId: resolvedTargetId, effect: effectIdByName(data, effect), sourceActionId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, participantIds: [pending.actorId, resolvedTargetId] } });
      if (expectedDamage > 0 && enemyFamily.markedSlow) {
        const marked = effectStateFor(target, "negative.помечен"), ownedMark = marked ? marked.sources.some(item => item.actorId === pending.actorId) : (target.effects || []).includes("negative.помечен");
         if (ownedMark) events.push({ type: "effect.apply", actorId: pending.actorId, payload: { targetId: resolvedTargetId, effect: "negative.замедлен", sourceActionId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, participantIds: [pending.actorId, resolvedTargetId] } });
      }
      ...
      if (expectedDamage > 0 && postDisplacement && (!postDisplacement.requiresSuccess || attackSucceeded)) {
        ...
        if (destination?.distance) {
          ...
          events.push({ type: "actor.move", actorId: target.id, payload: { space: target.space, x: destination.x, y: destination.y, movement: movementName, forced: true, displacement: { mode: "push", direction: destination.direction, distance: destination.distance, ruleId, sourceDigest: postDisplacement.sourceDigest || pending.techniqueSourceDigest || null }, path: (destination.path || []).map(cellKey), topologyCrossings: destination.topologyCrossings || [], participantIds: [pending.actorId, target.id] } });
  ...
  const enemyFamily = pending.enemyAttackFamily || {};
  if (enemyFamily.healerMark) for (const targetId of successfulEnemyTargets) events.push({ type: "effect.apply", actorId: pending.actorId, payload: { targetId, effect: "negative.помечен", sourceActionId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, duration: "scene", participantIds: [pending.actorId, targetId] } });
  if (enemyFamily.flux) for (const targetId of successfulEnemyTargets) events.push({ type: "effect.apply", actorId: pending.actorId, payload: { targetId, effect: "special.поток", sourceActionId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, duration: "scene", participantIds: [pending.actorId, targetId] } });
  ...
  if (successfulEnemyTargets.length && enemyFamily.createTerrainAdjacent && source) {
    ...
    for (const targetId of successfulEnemyTargets) {
      ...
      const hp = enemyTierFormula(enemyFamily.createTerrainAdjacent, source.tier);
       events.push({ type: "area.create", actorId: source.id, payload: { id: `enemy-terrain-${pending.id}-${targetId}`, space: source.space, areaType: "terrain", label: `${pending.name}: местность`, source: pending.actionId, ruleId: pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, duration: "scene", ownerActorId: source.id, cells: [`${cell.x},${cell.y}`], hp, maxHp: hp, participantIds: [source.id, targetId] } });
    }
  }
  ...
  if (source && enemyFamily.postTeleportFarthestAdjacent && successfulEnemyTargets[0]) {
    const target=actorById(scene,successfulEnemyTargets[0]);
    events.push({ type:"rule.prompt", actorId:source.id, payload:{ id:`prompt-${pending.id}-spright-teleport`, kind:"enemy-move-cell", sourceActorId:source.id, targetId:target.id, controller:"narrator", title:`${pending.name}: телепортация`, text:`Телепортируйте ${source.name} в самую дальнюю доступную клетку, смежную с ${target.name}.`, options:["cancel"], context:{teleportFarthestAdjacent:true,targetId:target.id}, participantIds:[source.id,target.id] } });
  }
  ...
  if (postSelfHeal && source) {
    ...
    if (amount > 0) events.push({ type: "actor.heal", actorId: source.id, payload: { targetId: source.id, amount, sourceActionId: pending.enemyRuleId || pending.actionId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId || null, sourceDigest: pending.sourceDigest || null, participantIds: [source.id, ...status.eligibleIds] } });
  }
  ...
  if (masterFinisher && source) {
    ...
    if (masterFinisher.modeId === "polearm" && (masterFinisher.spikedTargetIds || []).some(id => successful.has(id))) {
  ...
  events.push({ type: pending.enemyRuleId ? "enemy.action.resolve" : "action.resolve", actorId: pending.actorId, payload: pending.enemyRuleId ? { ruleId: pending.enemyRuleId, sourceRuleId: pending.sourceRuleId || pending.enemyRuleId, sourceDigest: pending.sourceDigest || null, name: pending.name, kind: "attack", targetIds: status.eligibleIds, skippedTargetIds: status.unavailableIds, reward: pending.reward || "" } : { actionId: pending.actionId, name: pending.name, attribute: pending.attribute || pending.roll?.attribute || null, roll: clone(pending.roll || null), targetIds: status.eligibleIds, targetCells: [...new Set(pending.targetCells || [])], skippedTargetIds: status.unavailableIds, masterArmament, techniqueRuleId:pending.techniqueRuleId||null, techniqueAnchor:clone(pending.techniqueAnchor||null), targetsTerrainCell:Boolean(pending.targetsTerrainCell), targetedTerrainId:pending.targetedTerrainId||null } });
