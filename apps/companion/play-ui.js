...
function renderChallengeRequestDock(){
  ...
  dock.hidden=!visible;if(visible)$("challenge-request-dock-text").textContent=participant?(result?`${result.successes} Успехов · ${opposedResultSummary(opposed)}`:`Встречный бросок против ${opponent?.name||"соперника"}`):request.result?`Результат принят · ${challengeResultSummary(request.result)}`:`${request.requestedBy} просит бросок · цель ${request.target}`;
}
...
function renderReference(){
  ...
  $("reference-list").innerHTML=list.slice(0,250).map(item=>`<article class="catalog-card"><span class="kind">${esc(item.kind)}</span><h3>${automationBadge(item.automationStatus)}${esc(item.name)}${item.cost?` · ${esc(item.cost)}`:""}</h3>${item.aliases?.length?`<div class="meta">Также: ${item.aliases.map(esc).join(" · ")}</div>`:item.tags?`<div class="meta">${esc(item.tags)}</div>`:""}<p>${md(item.text||"")}</p></article>`).join("")||`<p>Ничего не найдено.</p>`;
}
