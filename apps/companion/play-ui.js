...
function renderDiceComposer(){
  ...
  skill.innerHTML=`<option value="">Без Навыка</option>${S.skills.filter(item=>skillDisplayName(item).trim()).map(item=>`<option value="${item.id}">${esc(skillDisplayName(item))} · +${effectiveSkillRank(item)}D6</option>`).join("")}`;if([...skill.options].some(option=>option.value===skillValue))skill.value=skillValue;
