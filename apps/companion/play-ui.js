...
function referenceItems(){
  if(isEnglishPreview()&&isLionwingEdition())return[
    ...activeReferenceSections(),
    ...activeArchetypes().flatMap(archetype=>archetype.techniques.map(technique=>({...technique,kind:`Technique · ${archetype.name}`,text:[technique.flavor,...technique.levels.map(level=>`${level.n}: ${level.name} — ${level.text}`)].join("\n")}))),
    ...activeOutlooks().flatMap(outlook=>(outlook.builtin?[outlook.builtin]:[]).concat(outlook.gifts).map(gift=>({...gift,kind:`Boon · ${outlook.name}`}))),
  ];
  const items=[...RULES,...GLOSSARY,...bondRuleItems(),...D.bonds.actions.map(x=>({...x,kind:x.antagonistic?"Связь · антагонистическое действие":"Связь · действие",tags:x.tag})),...D.actions.list.map(x=>({...x,kind:"Действие"})),...D.effects.positive.map(x=>({...x,kind:"Положительный эффект"})),...D.effects.negative.map(x=>({...x,kind:"Отрицательный эффект"})),...D.archetypes.flatMap(a=>a.techniques.map(t=>{const statuses=t.levels.map(level=>techniqueLevelAutomation(t.id,level.n)),automated=statuses.filter(status=>status!=="manual").length;return{...t,automationStatus:automated===statuses.length&&statuses.every(status=>status==="full")?"full":automated?"partial":"manual",kind:`Техника · ${a.name}`,text:[t.flavor,...t.levels.map(l=>`${l.n}: ${l.name} — ${l.text}`)].join("\n")}})),...D.outlooks.flatMap(o=>(o.builtin?[o.builtin]:[]).concat(o.gifts).map(g=>({...g,kind:`Дар · ${o.name}`}))),...enemyProfiles().map(enemy=>({...enemy,automationStatus:enemyProfileAutomation(enemy),kind:enemy.kind==="modifier"?"Враг-Модификатор":"Враг",text:`${enemy.statsRaw}\n${enemy.examples}\n${enemy.text}`})),...(D.enemies?.antagonistTraits||[]).map(trait=>({...trait,kind:"Черта Антагониста",text:trait.rules.map(rule=>`${rule.name} (${rule.trigger}): ${rule.text}`).join("\n")}))];
  return items;
}
function renderReference(){
  const en=isEnglishPreview()&&isLionwingEdition(),q=$("ref-search").value.trim().toLowerCase(),filters=en?["all","Builder Reference","Technique","Boon"]:["all","Термин","Памятка","Связь","Действие","Эффект","Техника","Дар","Враг"],tagList=item=>String(item.tags||"").split(",").map(tag=>tag.trim()).filter(Boolean);
...
function setMode(mode){store.mode=["build","play","tools","rules","reference"].includes(mode)?mode:"build";if(isEnglishPreview()&&["play","tools","rules"].includes(store.mode))store.mode="build";if(store.mode!=="play")setScenePanel(null);document.body.classList.toggle("builder-mode",store.mode==="build");document.body.classList.toggle("reference-mode",store.mode==="reference");document.body.classList.toggle("scene-mode",store.mode==="play");document.body.classList.toggle("scene-player-view",store.mode==="play"&&activeSceneView()==="player");if(store.mode!=="tools")document.body.classList.remove("tools-narrator-mode");$$('[data-page]').forEach(p=>p.classList.toggle("active",p.dataset.page===store.mode));$$('[data-mode]').forEach(b=>b.setAttribute("aria-current",b.dataset.mode===store.mode?"page":"false"));if(store.mode==="play")renderPlay();if(store.mode==="tools")renderToolsWorkspace();if(store.mode==="rules")renderRules();if(store.mode==="reference")renderReference();renderChallengeRequestDock();persist();if(typeof syncContentUrl==="function")syncContentUrl();if(["rules","reference"].includes(store.mode)&&location.hash)requestAnimationFrame(()=>document.getElementById(location.hash.slice(1))?.scrollIntoView());else window.scrollTo({top:0,behavior:"smooth"});}
