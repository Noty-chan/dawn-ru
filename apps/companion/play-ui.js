...
function renderReference(){
  const lionwing=isLionwingEdition(),en=isEnglishPreview()&&lionwing,q=$("ref-search").value.trim().toLowerCase(),filters=lionwing?(en?["all","Builder Reference","Rule","Action","Effect","Technique","Boon"]:["all","Справка","Правило","Действие","Эффект","Техника","Дар"]):["all","Термин","Памятка","Связь","Действие","Эффект","Техника","Дар","Враг"],tagList=item=>String(item.tags||"").split(",").map(tag=>tag.trim()).filter(Boolean);
