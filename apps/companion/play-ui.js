...
function renderPlay(){
  ensureRuntime();const d=derived(),rt=S.runtime,usesWounds=d.guts!=null;
  $("ko-banner").innerHTML=usesWounds&&rt.wounds>=d.guts?`<div class="ko"><strong>Раны догнали Стойкость.</strong> Герой выводится из строя: снимите одну Рану и увеличьте Напряжение — либо поставьте Влияние на кон и вернитесь с 2 Стресса и Здоровьем, равным Стойкости.<div class="button-row"><button type="button" id="ko-yield">Выведен из строя</button><button type="button" id="ko-stake">Поставить на кон</button></div></div>`:"";
  $("play-counters").innerHTML=counter("hp","Здоровье",rt.hp,d.hp)+(usesWounds?counter("wounds","Раны",rt.wounds,d.guts):"")+counter("focus","Фокус",rt.focus)+counter("ap","ОД",rt.ap)+counter("influence","Влияние",rt.influence)+counter("stress","Стресс",rt.stress,3)+counter("tension","Напряжение",rt.tension)+(hasGift("Trust Fund")?counter("funding","Финансирование",rt.funding):"");
...
function renderOutcomeGuide(){
  ...
  $("dice-outcome-guide").innerHTML=`<span><b>${copy.failure}</b> · 0–${target-1}</span><span><b>${copy.minimal}</b> · ${target}${minimumEnd>target?`–${minimumEnd}`:""}</span><span><b>${copy.extreme}</b> · ${target*2}+</span>`;
}
...
function renderEnglishToolsDirector(role,kind,request,opposed){
  ...
  if(role==="local-table"){$("freeplay-director-kind").textContent="LOCAL TABLE";$("freeplay-director-title").textContent=kind==="opposed"?"Opposed Roll on one device":"Challenge on one device";$("freeplay-director-help").textContent=kind==="opposed"?"Choose both sides. Heroes use their sheets; the Narrator can roll manually for an NPC.":"Choose a local sheet and Success Target, then build the pool.";state.innerHTML=kind==="opposed"?`<strong>${opposed?esc(opposed.participants.map(item=>item.name).join(" vs ")):"Prepare both sides"}</strong><span>The side with more Successes wins; a tie starts a joint reroll.</span>`:"<strong>Local play</strong><span>Changing the sheet source uses that hero's complete local sheet.</span>"}
  else if(role==="network-narrator"){$("freeplay-director-kind").textContent="SHARED TABLE";$("freeplay-director-title").textContent="Narrator desk";$("freeplay-director-help").textContent=kind==="opposed"?"Choose both sides. Hero owners receive a request; roll manually for an NPC.":"Choose a request recipient and assign a Success Target. Your local sheet stays available below.";state.innerHTML=request?.result?`<strong>Result received: ${request.result.successes} Successes · ${request.result.crits} Crits</strong><span>${esc(actor?.name||"Hero")} · ${request.result.rolls.join(" · ")}</span>`:request?`<strong>Waiting for ${esc(actor?.name||"hero")}</strong><span>Assigned Success Target: ${request.target}.</span>`:"<strong>No active request</strong><span>A request is optional; you can still roll from your local sheet.</span>"}
  else{$("freeplay-director-kind").textContent=side||own?"NARRATOR REQUEST":"FREE ROLL";$("freeplay-director-title").textContent=side?"Opposed Roll":own?"Assigned challenge":"Roll without a request";$("freeplay-director-help").textContent=side?"Build the pool from your sheet and roll for your side.":own?"Build the pool from your sheet and make a public roll.":"Build any pool; the result appears in the shared roll feed.";state.innerHTML=side?`<strong>${sideResult?`Your result: ${sideResult.successes} Successes`:"Your result is needed"}</strong><span>${esc(opposedResultSummary(opposed))}</span>`:own?`<strong>Success Target: ${own.target}</strong><span>${esc(own.requestedBy)} requested a roll for ${esc(S.name||"your hero")}.</span>`:"<strong>Free public roll</strong><span>No Narrator request is required.</span>"}
...
function renderDiceComposer(){
  const skill=$("dice-skill"),ability=$("dice-ability"),bond=$("dice-bond"),skillValue=skill.value,abilityValue=ability.value,bondValue=bond.value,darkChecked=$("dice-dark-urge")?.checked,outgunnedChecked=$("dice-outgunned")?.checked,copy=toolsCopy();
...
function updateAllInAvailability(){
  ...
  const copy=toolsCopy();$("all-in-hint").textContent=!pendingAllIn?copy.allInFirst:influence>0?copy.allInInfluence:stressPayment&&stress<3?copy.allInStress:copy.allInBlocked;
}
...
function rollDice(){
  ...
  if(!status.available)return toast(status.reason);pendingAllIn={count,diceRequest:request,scenario};if(!resolveDice(count,4,"",request,scenario)){pendingAllIn=null;return}updateAllInAvailability();
}
...
const PINNED_RULE_LIMIT=3;
let rulePinRecords=new Map();
function pinnedRules(){if(!Array.isArray(store.pinnedRules))store.pinnedRules=[];return store.pinnedRules}
function makeRulePinRecord(item,source="rules",context="item",anchor=""){
  const raw=ruleCardData(item)||{},title=String(raw.name||raw.title||t("heroView.pinned.ruleFallback")),text=String(raw.text||raw.description||""),key=String(raw.id||ruleKey(`${title}:${text}`)),id=`${source}:${context}:${key}`,record={id,key,source,context,anchor,title,text,kind:String(raw.kind||raw.category||raw.group||t(source==="reference"?"heroView.pinned.reference":"heroView.pinned.rules"))};rulePinRecords.set(id,record);return record;
}
function rulePinButton(record){const pinned=pinnedRules().some(item=>item.id===record.id),label=t(pinned?"heroView.pinned.unpin":"heroView.pinned.pin");return`<button type="button" class="rule-pin ${pinned?"on":""}" data-pin-rule="${esc(record.id)}" aria-pressed="${pinned}" title="${esc(label)}"><span aria-hidden="true">${pinned?"★":"☆"}</span><span class="sr-only">${esc(label)}</span></button>`}
function togglePinnedRuleById(id){
  const record=rulePinRecords.get(id);if(!record)return toast(t("heroView.pinned.missing"));
  const list=pinnedRules(),index=list.findIndex(item=>item.id===record.id);
  if(index>=0){list.splice(index,1);persist();refreshPinnedRuleSurfaces();return}
  if(list.length>=PINNED_RULE_LIMIT)return toast(t("heroView.pinned.limit",{limit:PINNED_RULE_LIMIT}));
  list.push({...record});persist();refreshPinnedRuleSurfaces();
}
function refreshPinnedRuleSurfaces(){
  if(store.mode==="rules")renderRules();if(store.mode==="reference")renderReference();if(typeof renderHeroPlaySheet==="function"&&document.querySelector("#hero-play-sheet"))renderHeroPlaySheet();
}
function openPinnedRule(id){
  const record=pinnedRules().find(item=>item.id===id);if(!record)return;
  setMode(record.source==="reference"?"reference":"rules");requestAnimationFrame(()=>{const target=record.anchor&&document.getElementById(record.anchor);if(!target)return;target.closest(".rules-chapter")?.setAttribute("open","");target.scrollIntoView({behavior:"smooth",block:"center"})});
}
function pinnedRulesMarkup(){
  const list=pinnedRules();list.forEach(item=>rulePinRecords.set(item.id,item));const count=t("heroView.pinned.count",{count:list.length,limit:PINNED_RULE_LIMIT});
  const cards=list.map(item=>`<article class="hero-sheet-pinned-rule"><header><div><span>${esc(item.kind||t("heroView.pinned.rules"))}</span><h3>${esc(item.title||t("heroView.pinned.ruleFallback"))}</h3></div><button type="button" class="hero-sheet-pinned-remove" data-pinned-rule-remove="${esc(item.id)}" aria-label="${esc(t("heroView.pinned.unpin"))}">×</button></header><div>${md(item.text||"")}</div><footer><button type="button" data-pinned-rule-open="${esc(item.id)}">${esc(t(item.source==="reference"?"heroView.pinned.openReference":"heroView.pinned.openRules"))}</button></footer></article>`).join("");
  return `<details class="hero-sheet-card hero-sheet-pinned-rules" open><summary><span><span class="eyebrow">05</span><b>${esc(t("heroView.pinned.title"))}</b></span><small>${esc(count)}</small></summary><p class="hero-sheet-pinned-help">${esc(t("heroView.pinned.help"))}</p><div class="hero-sheet-pinned-list">${cards||`<div class="hero-sheet-pinned-empty"><p>${esc(t("heroView.pinned.empty"))}</p><div class="button-row compact"><button type="button" data-pinned-rules-mode="rules">${esc(t("heroView.pinned.chooseRules"))}</button><button type="button" data-pinned-rules-mode="reference">${esc(t("heroView.pinned.chooseReference"))}</button></div></div>`}</div></details>`;
}
function ruleCardsHtml(cards,chapterId="rule"){
  const linkLabel=isEnglishPreview()?"Link to rule":"Ссылка на правило";
  return `<div class="rules-card-grid">${cards.map(card=>{const {name,text}=ruleCardData(card),id=ruleCardId(card,chapterId),pin=makeRulePinRecord({...ruleCardData(card),kind:ruleCardData(card).category||t("heroView.pinned.rules")},"rules",chapterId,id);return `<article id="${id}" class="rules-card"><header><h3>${esc(name)}</h3><div class="rules-card-links">${rulePinButton(pin)}<a class="rule-permalink" href="#${id}" aria-label="${linkLabel} «${esc(name)}»">#</a></div></header><div>${md(text)}</div></article>`}).join("")}</div>`;
}
...
function combatActionReferenceHtml(query=""){
  ...
  return `<section class="combat-action-reference"><header><span class="kind">БАЗОВЫЕ ДЕЙСТВИЯ</span><h3>Все варианты прямо в разделе боя</h3><p>Атаки, защитные Реакции, движение и утилитарные действия приведены здесь полностью — переходить в другую главу не требуется.</p></header>${groups.map(group=>`<details open><summary>${esc(group)} <b>${actions.filter(action=>action.group===group).length}</b></summary><div class="rules-card-grid">${actions.filter(action=>action.group===group).map(action=>{const id=`rule-combat-action-${ruleKey(`${action.name}:${action.text}`)}`,pin=makeRulePinRecord({...action,kind:`Действие · ${group}`},"rules","combat",id);return `<article id="${id}" class="rules-card"><header><h3>${esc(action.name)}</h3>${action.cost?`<span>${esc(action.cost)}</span>`:""}<div class="rules-card-links">${rulePinButton(pin)}<a class="rule-permalink" href="#${id}" aria-label="Ссылка на действие «${esc(action.name)}»">#</a></div></header><div>${md(action.text)}</div></article>`}).join("")}</div></details>`).join("")}</section>`;
}
