...
function updateDicePoolTotal(){const base=clamp($("dice-count").value,1,40),adv=clamp($("dice-adv").value,0,30),dis=clamp($("dice-dis").value,0,30),total=Math.max(1,base+adv-dis);$("dice-pool-total").innerHTML=`Бросок: <strong>${total}D6</strong><small>${base} базовых${adv?` · +${adv} Преимущество`:""}${dis?` · −${dis} Помеха`:""}</small>`;$$("[data-dice-count]").forEach(button=>button.classList.toggle("on",+$("dice-count").value===+button.dataset.diceCount))}
function recalculateDicePool(){const attr=$("dice-attr").value,skill=S.skills.find(item=>item.id===$("dice-skill").value),abilityKey=$("dice-ability").value,ability=abilityKey==="tainted"?S.taintedAbility:abilityKey==="main"?S.ability:null,count=$("dice-count");count.readOnly=attr!=="manual";if(attr!=="manual")count.value=Math.max(1,attrValue(attr)+(skill?effectiveSkillRank(skill):0)+(ability?.enabled?ability.rank:0));updateDicePoolTotal()}
function renderDiceComposer(){const skill=$("dice-skill"),ability=$("dice-ability"),skillValue=skill.value,abilityValue=ability.value;skill.innerHTML=`<option value="">Без Навыка</option>${S.skills.filter(item=>item.name.trim()).map(item=>`<option value="${item.id}">${esc(item.name)} · +${effectiveSkillRank(item)}D6</option>`).join("")}`;if([...skill.options].some(option=>option.value===skillValue))skill.value=skillValue;ability.innerHTML=`<option value="">Без Способности</option>${S.ability.enabled?`<option value="main">${esc(S.ability.name||"Способность")} · +${S.ability.rank}D6</option>`:""}${S.mods.taintedBody&&S.taintedAbility.enabled?`<option value="tainted">${esc(S.taintedAbility.name||"Порченое тело")} · +${S.taintedAbility.rank}D6</option>`:""}`;if([...ability.options].some(option=>option.value===abilityValue))ability.value=abilityValue;$("dice-hero-context").innerHTML=`<span>Бросает</span><strong>${esc(S.name||"Безымянный герой")}</strong><small>Ступень ${S.tier}</small>`;recalculateDicePool()}
