...
const lionwingTriggerTension = scene => Number(globalThis.DAWN_LIONWING_COMBAT_METER?.read?.(scene)?.current ?? scene?.tension ?? 0);
...
function triggeredEvents(scene, event, options = {}) {
  ...
  if (event.type === "action.resolve" && actor && actionIdIs(eventActionId(payload), "charge") && Number(actor.techniques?.["vagabond.egomaniac"] || 0) >= 3 && clockStatus(scene, actor.id, "vagabond.egomaniac.style").active && lionwingTriggerTension(scene) > 0 && !scene.pendingPrompt && !promptQueued()) events.push({ type: "rule.prompt", actorId: actor.id, payload: { id: `prompt-${event.id}-finale`, kind: "egomaniac-finale", sourceActorId: actor.id, title: "Финал", text: `Заполнить ${lionwingTriggerTension(scene) * 2} сегмента Стиля с переносом между заполнениями, затем потерять Стиль до конца Сцены?`, options: ["finale", "pass"], participantIds: [actor.id] } });
  ...
  if (event.type === "action.resolve" && actor && Number(actor.techniques?.["disruptor.chemist"] || 0) >= 1 && (payload.targetedTerrainId || payload.targetsTerrainCell) && payload.techniqueAnchor && !scene.pendingPrompt && !promptQueued()) {
   ...
   if (event.type === "action.resolve" && actor && actionIdIs(eventActionId(payload), "charge") && Number(actor.techniques?.["ruiner.grim-ascendant"] || 0) >= 1 && lionwingTriggerTension(scene) >= 2 && !actor.ruleState?.grimUsed && !scene.pendingPrompt && !promptQueued()) {
   ...
   if (event.type === "action.resolve" && actor && actionIdIs(eventActionId(payload), "charge") && Number(actor.techniques?.["powerhouse.warring-ascendant"] || 0) >= 1 && lionwingTriggerTension(scene) >= 2 && !actor.ruleState?.warringUsed && !scene.pendingPrompt && !promptQueued()) {
