...
const TRIGGER_RULES = [
  ...
  },
  {
    ...
    priority: 60,
    match: ({ scene, actor, payload }) => {
      if (!actor || !actionIdIs(eventActionId(payload), "study") || Number((actor.knownTechniques ?? actor.techniques)?.["disruptor.siren"] || 0) < 1) return false;
...
function triggeredEvents(scene, event, options = {}) {
  ...
  if (event.type === "resource.gain" && actor && payload.resolvedResource === "focus" && Number(payload.resolvedDelta || 0) > 0 && Number(actor.techniques?.["ruiner.cryomancer"] || 0) >= 2 && typeof payload.actionInstanceId === "string" && !(scene.log || []).some(logged => logged.type === "rule-clock.tick" && logged.actorId === actor.id && logged.payload?.clockId === "ruiner.cryomancer.icicle" && logged.payload?.actionInstanceId === payload.actionInstanceId)) events.push({ type: "rule-clock.tick", actorId: actor.id, payload: { clockId: "ruiner.cryomancer.icicle", delta: 1, actionInstanceId: payload.actionInstanceId, sourceActionId: "ruiner.cryomancer.2", reason: "Получен Фокус" } });
  ...
  if (event.type === "action.resolve" && actor && actionIdIs(eventActionId(payload), "breathe") && Number(actor.techniques?.["ruiner.cryomancer"] || 0) >= 2 && clockStatus(scene, actor.id, "ruiner.cryomancer.icicle").value > 0 && !scene.pendingPrompt && !promptQueued()) {
    ...
    if (gain) events.push({ type: "rule.prompt", actorId: actor.id, payload: { id: `prompt-${event.id}-icicle`, kind: "cryomancer-icicle-rest", sourceActorId: actor.id, title: "Ледяной нимб", text: "До Передышки выбрать цель в пределах 5: отказаться от эффекта Передышки, очистить Сосульку и нанести цели урон Дух/2 за каждый сегмент.", options: ["convert", "pass"], context: { actionInstanceId: payload.actionInstanceId, focusGainEventId: gain.id }, participantIds: [actor.id] } });
  }
  ...
  if (event.type === "action.resolve" && actor && actionIdIs(eventActionId(payload), "spell") && Number(payload.roll?.successes || 0) > 0 && Number(actor.techniques?.["ruiner.cryomancer"] || 0) >= 1) for (const targetId of payload.targetIds || []) events.push({ type: "effect.apply", actorId: actor.id, payload: { targetId, effect: "negative.замедлен", sourceActionId: "ruiner.cryomancer.1", participantIds: [actor.id, targetId] } });
  ...
  if (event.type === "action.resolve" && actor && actionIdIs(eventActionId(payload), "charge") && Number(actor.techniques?.["ruiner.grim-ascendant"] || 0) >= 1 && Number(scene.tension || 0) >= 2 && !actor.ruleState?.grimUsed && !scene.pendingPrompt && !promptQueued()) {
    events.push({ type: "rule.prompt", actorId: actor.id, payload: { id: `prompt-${event.id}-grim`, kind: "grim-transform", sourceActorId: actor.id, title: "Непостоянная мощь", text: "Трансформироваться: получить 1 ОД, потерять Фокус и всё Здоровье кроме 1 и оттолкнуть врагов в дальности 2?", options: ["transform", "pass"], participantIds: [actor.id] } });
  }
  ...
  if (event.type === "turn.end") {
    for (const privateer of (scene.actors || []).filter(item => !item.knockedOut && ["enemy.common.privateer","lionwing.npc.privateer"].includes(item.profileId) && item.ruleState?.privateerGearChange && effectPresenceStatus(scene, item.id).onField)) {
