...
const lines = [
  ...
  "| --- | ---: | ---: | ---: | --- |",
  `| Уровни Техник | ${coverage.length} | ${executableTechniqueLevels} shared/inherited (${percent(executableTechniqueLevels, coverage.length)}) | ${certifiedEvidence.filter(entry => entry.kind === "technique").length} | ${techniqueCounts.partial || 0} частичных; ${techniqueCounts.manual || 0} ручных |`,
  ...
  "",
  `Формально сертифицировано сейчас: **${certifiedEvidence.length}** записей. Исполнимые уровни без evidence считаются **shared/inherited**, даже при совпавшем canonical digest: digest подтверждает текстовый источник, но не полноту поведения. Проценты выше измеряют охват кодом, а не верность правилам игры.`,
  "",
