...
| --- | ---: | ---: | ---: | --- |
| Уровни Техник | 321 | 66 (20.6%) | 0 | 45 частичных; 210 ручных |
| Правила обычных врагов | 122 | 60 (49.2%) | 0 | 62 assisted |
...
| --- | ---: | ---: | ---: | ---: | ---: |
| Силач | 54 | 7 | 3 | 6 | 38 |
| Бродяга | 57 | 7 | 10 | 7 | 33 |
| Оплот | 51 | 3 | 3 | 3 | 42 |
| Альтруист | 51 | 1 | 8 | 6 | 36 |
| Подрывник | 54 | 4 | 5 | 12 | 33 |
| Разрушитель | 54 | 3 | 12 | 11 | 28 |
...
| --- | ---: | --- |
| `movement-lifecycle` · Жизненный цикл движения | 94 | Один общий контракт сможет снять этот блокер сразу у нескольких адаптеров; модуль пока не закреплён |
| `turn-lifecycle` · Жизненный цикл Хода и Раунда | 63 | Один общий контракт сможет снять этот блокер сразу у нескольких адаптеров; модуль пока не закреплён |
| `derived-stats` · Производные характеристики персонажа | 38 | Один общий контракт сможет снять этот блокер сразу у нескольких адаптеров; модуль пока не закреплён |
| `scene-lifecycle` · Начало, конец и сброс Сцены | 28 | Один общий контракт сможет снять этот блокер сразу у нескольких адаптеров; модуль пока не закреплён |
| `inventory` · Инвентарь и заряды | 23 | Один общий контракт сможет снять этот блокер сразу у нескольких адаптеров; модуль пока не закреплён |
...
| `deployment-hooks` · Развертывание | 6 | Один общий контракт сможет снять этот блокер сразу у нескольких адаптеров; модуль пока не закреплён |
...
| Капер (`enemy.common.privateer`) | 3 | 1 | 2 | Эскорт; Смена снаряжения |
