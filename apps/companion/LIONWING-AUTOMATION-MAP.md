...
| --- | ---: | --- |
| `parameterize-old-core` | 3 | Существующий общий контракт подходит после явного выбора редакции и новых параметров. |
| `extend-shared-core` | 3 | Нужен новый общий контракт, который не меняет поведение 0.9. |
| `new-lionwing-foundation` | 6 | Простой патч старой автоматики недостаточен; нужен отдельный типизированный фундамент LionWing. |
...
| Параллельная Сцена Дуэли (`lionwing.foundation.detached-duel-scenes`) | `planned` | дочерняя Сцена; синхронное продолжение основной Сцены; ставка; возврат участников; отдельное Напряжение |
| Сопротивление Выведению и Уязвимость (`lionwing.foundation.resisting-vulnerable`) | `source-ready-data-pending` | состояние Уязвимости до конца Сцены; запрет повторного Сопротивления; запрет Влияния; одноразовые необратимые последствия |
...
| --- | --- | --- | --- | --- |
| `lionwing.rules.bilingual-source` | Двуязычный источник Правил и Справочника | расширить общее ядро | `in-progress` | 34–65 |
...
| `lionwing.foundation.resisting-vulnerable` | Сопротивление Выведению и Уязвимость | новый фундамент LionWing | `source-ready-data-pending` | 38 |
| `lionwing.core.quick-roll` | Быстрый бросок | параметризовать общее ядро | `source-ready-data-pending` | 36 |
...
| `lionwing.builder.fixed-skills` | Канонический список Навыков и произвольные Навыки | расширить общее ядро | `source-ready-data-pending` | 44 |
