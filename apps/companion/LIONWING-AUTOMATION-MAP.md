...
| --- | ---: | --- |
| `parameterize-old-core` | 10 | Существующий общий контракт подходит после явного выбора редакции и новых параметров. |
...
| Профили Support NPC LionWing (`lionwing.npcs.support-profiles`) | `8-profiles-data-ready` | цепное перемещение союзников с лимитом на цель; сменная Атака от другого NPC; превращение Местности в Приспешников; Действия союзника в Ход Координатора; динамическое копирование профиля; копии с общими начальными параметрами; Страж и геометрия выбора цели; Поток для временного обмена персонажей; безопасные назначенные клетки; автоматическая Жертва |
| Профили Engine NPC LionWing (`lionwing.npcs.engine-profiles`) | `11-profiles-data-ready` | запрет выбранных Действий с перехватом цели; обязательная клетка к концу следующего Хода; порог урона одним случаем; изменяемый Пассив; Часы Подготовки; Козырь Н0 с повторным применением и тремя случаями урона; счетчики Рока и виртуальное Напряжение; замена Культиста усиленным NPC; движение при росте Напряжения; принудительный Ход против союзника; Ищейки с целью и взрывом; Трупы и оживление как NPC меньшей Ступени; движение сопровождения вслед за союзником; сеть Разломов с удалением и лимитом раз за Ход; распределенный NPC из Зон Приспешников |
| Профили Модификаторов LionWing (`lionwing.modifiers.profiles`) | `12-profiles-data-ready` | указанные Зоны с переносом в конце Раунда; критический порог и выбранный вариант Пыла; дружественные NPC без Ходов и условия поражения; правила недоступности VIP и Случайных жертв для цели; деление группового урона Изоляции; поочередное переключение Внутрь и Наружу; расширение поля и занимающий край Громадина; дальность от созданной Местности; занимающий область 2 × 2 Носитель; возврат несоставных NPC с половиной Здоровья; Зоны Приспешников с ограниченным направлением движения; счетчик Брони только от конкретного эффекта |
...
| `lionwing.builder.ability-expansion` | Расширение одной Способности новыми словами | расширить общее ядро | `data-ready` | 45 |
| `lionwing.core.bonds` | Жизненный цикл и Действия Связей LionWing | параметризовать общее ядро | `data-ready` | 47–48 |
| `lionwing.foundation.antagonism-actions` | Антагонизм и Действия Антагониста | новый фундамент LionWing | `data-ready` | 106 |
...
| `lionwing.npcs.support-profiles` | Профили Support NPC LionWing | новый фундамент LionWing | `8-profiles-data-ready` | 120–121 |
