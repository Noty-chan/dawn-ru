...
function validateEvent(scene, event, options = {}) {
  ...
  if (event.type === "actor.move") {
    ...
    if(actor?.profileId==="lionwing.npc.cocoon"&&actor.ruleState?.cocoonAwake!==true&&!payload.forced&&!payload.placement)throw new Error("Смирный Кокон не может двигаться.");
    if (actor?.kind === "crowd") {
  ...
  if (event.type === "actor.spawn") {
    ...
    if (spawned.crowdSubtype === "seeker") {
      ...
      if (!owner || event.actorId !== owner.id || !["enemy.common.hound-master","lionwing.npc.hound-master"].includes(owner.profileId) || owner.knockedOut || !target || target.knockedOut || target.kind === "crowd" || target.team === owner.team || spawned.kind !== "crowd" || spawned.team !== owner.team || spawned.space !== owner.space || distance(owner, spawned) !== 1 || !["enemy.common.hound-master.action.fire-seeker", "enemy.common.hound-master.trump.wild-hunt","lionwing.npc.hound-master.fire-seeker","lionwing.npc.hound-master.wild-hunt"].includes(ruleId) || Number(spawned.seekerDamage) !== enemyTierFormula("7(+1)", owner.tier)) throw new Error("Ищейка не соответствует авторитетному правилу Псаря.");
    }
...
const expectedTargets = (scene.actors || []).filter(target => !target.knockedOut && target.id !== source.id && target.team !== source.team && target.space === source.space && geometry.footprint(target).some(cell => geometry.cells.includes(cell))).map(target => target.id).sort();
  ...
  if (event.type === "rule.respond") {
    ...
    if (event.actorId !== prompt.sourceActorId || !source || source.knockedOut && !prompt.context?.revenantReturn) throw new Error("Источник решения больше не доступен.");
  ...
  if (event.type === "actor.state") {
    ...
    if (["martialPerfection", "imposingPresence", "berserkerLastStand", "cocoonAwake"].includes(payload.key) && typeof payload.value !== "boolean") throw new Error("Некорректный переключатель состояния.");
