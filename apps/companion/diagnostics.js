...
(() => {
  ...
  function targetDescription(target){
    ...
    const dataset=Object.fromEntries(Object.entries(element.dataset||{}).filter(([key])=>/^(mode|action|enemyRule|scenePanel|openScenePanel|sceneTool|sceneAreaType|sceneTurnActor|heroView|tech|level)/i.test(key)).slice(0,10));
  ...
  if(typeof commitSceneEvents==="function"){
    const original=commitSceneEvents;commitSceneEvents=function(label,events){const geometry=(events||[]).filter(event=>/^(wall|area|topology|actor\.move)/.test(event.type)).map(event=>({type:event.type,actorId:event.actorId||null,payload:scrub(event.payload||{})}));record("scene.commit",label,{round:Scene?.round,version:Scene?.version,eventTypes:(events||[]).map(event=>event.type),actorIds:[...new Set((events||[]).map(event=>event.actorId).filter(Boolean))],geometry});try{const result=original(label,events);record(result?"scene.commit.ok":"scene.commit.rejected",label,{version:Scene?.version});return result}catch(error){record("scene.commit.error",label,{message:error.message,stack:error.stack});throw error}};
  }
  if(typeof commitScene==="function"){
    const original=commitScene;commitScene=function(label,mutator,options){record("scene.manual",label,{round:Scene?.round,version:Scene?.version,options:scrub(options||{})});try{const result=original(label,mutator,options);record(result?"scene.manual.ok":"scene.manual.rejected",label,{version:Scene?.version});return result}catch(error){record("scene.manual.error",label,{message:error.message,stack:error.stack});throw error}};
  }
  ...
  function compactDiagnostics(source){
    const counts=new Map();for(const row of source){const signature=`${row.kind}\n${row.summary}\n${JSON.stringify(row.details||{})}`;const item=counts.get(signature)||{count:0,firstAt:row.at,lastAt:row.at,row};item.count++;item.lastAt=row.at;counts.set(signature,item)}
    const emitted=new Set(),result=[];for(const row of source){const signature=`${row.kind}\n${row.summary}\n${JSON.stringify(row.details||{})}`,item=counts.get(signature);if(item.count<4){result.push(row);continue}if(emitted.has(signature))continue;emitted.add(signature);result.push({...item.row,at:item.firstAt,lastAt:item.lastAt,repeat:item.count})}return result;
  }
  function diagnosticHighlights(source){
    const failures=source.filter(row=>/error|rejection|rejected/i.test(row.kind)||/отклон|ошиб|не удалось|invalid|failed/i.test(row.summary)).slice(-16).map(row=>({at:row.at,kind:row.kind,summary:row.summary,details:row.details})),notices=new Map();for(const row of source.filter(row=>row.kind==="ui.notice")){const item=notices.get(row.summary)||{summary:row.summary,count:0,firstAt:row.at,lastAt:row.at};item.count++;item.lastAt=row.at;notices.set(row.summary,item)}return{failures,frequentNotices:[...notices.values()].filter(item=>item.count>=3).sort((a,b)=>b.count-a.count).slice(0,12)};
  }
  function reportData(description="",steps="",includeState=true){
    const navigation=performance.getEntriesByType?.("navigation")?.[0];
    const recent=rows.slice(-350);return{reportSchema:2,createdAt:now(),description:String(description).trim(),steps:String(steps).trim(),environment:{url:`${location.origin}${location.pathname}${location.search}`,locale:document.documentElement.lang,edition:contentPreferences?.edition,mode:store?.mode,viewport:`${innerWidth}x${innerHeight}`,userAgent:navigator.userAgent,online:navigator.onLine,loadType:navigation?.type||null},highlights:diagnosticHighlights(recent),diagnostics:copy(compactDiagnostics(recent)),...(includeState?{hero:heroSummary(),scene:sceneSummary(),sceneLog:recentSceneLog()}: {})};
  }
