...
(function exposeDawnTechniqueEngine(global) {
  ...
  const RULES = [
    ...
    { id: "disruptor.siren.2", techniqueId: "disruptor.siren", level: 2, name: "Неотразимая", kind: "passive", automation: "decision", note: "Первое за Ход наложение Испуган открывает отменяемый выбор клетки: путь цели до 3 клеток проверяется пошагово на приближение к Сирене, смежный итог накладывает Ошеломлен и всё фиксируется в журнале." },
  ...
  function preview(scene, request = {}) {
    ...
    if (rule.kind === "combo") {
      const prepared = global.DAWN_SCENE_ENGINE?.prepareTechniqueCombo(scene, global.DAWN_DATA, { actorId: actor.id, ruleId: rule.id, targetIds: request.targetIds || [], destination: request.destination || null, roll: request.roll || null, attribute: request.attribute || request.roll?.attribute || null });
