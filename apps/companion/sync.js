...
(function exposeDawnSync(global){
  ...
  function fail(error){const raw=error?.message||String(error||"Ошибка синхронизации"),timedOut=String(error?.code||"")==="57014"||/statement timeout|canceling statement/i.test(raw),message=timedOut?"Сервер не успел обработать изменение; соединение восстанавливается, действие будет повторено":raw;if(timedOut)scheduleReconnect("statement-timeout");else patch({status:"error",error:message});const failure=error instanceof Error?error:new Error(message);if(timedOut)failure.message=message;throw failure}
