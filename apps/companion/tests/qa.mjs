...
assert.match(appSource, /const inSpace=[\s\S]+base\.objects=base\.objects\.map[\s\S]+base\.topology\.cuts=/, "Imported actors and field entities are repaired to their actual space bounds");
...
const emptyTargetActionScene = vm.runInContext(`sceneCore({spaces:[{id:"main",width:7,height:7}],actors:[{id:"hero",team:"hero",space:"main"}],pendingAction:{id:"pending",actorId:"hero",targetIds:[],allowEmptyTargets:true,responses:{}}})`, appCoreContext);
