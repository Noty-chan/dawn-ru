...
assert.match(appSource, /actor\.rulesEdition==="lionwing"[^\n]+actor\.guts=null;actor\.wounds=clamp\(source\.wounds,0,3\)/, "LionWing must retain its three-Wound track without introducing legacy Guts");
...
assert.match(companionMarkup, /id="scene-interface-next-styles"[^>]+disabled/, "The redesigned stylesheet must start inert until the inline rollout chooses exactly one table interface");
assert.match(appSource, /interfaceVersion:sceneInterfaceVersion/, "The selected table interface version must persist locally");
assert.match(appSource, /SCENE_INTERFACE_ROLLOUT_VERSION=3[\s\S]+sceneInterfaceRolloutVersion<SCENE_INTERFACE_ROLLOUT_VERSION\?"next"[\s\S]+interfaceRolloutVersion:SCENE_INTERFACE_ROLLOUT_VERSION/, "The new table must become the default once without removing the classic opt-out");
assert.match(appSource, /\$\("scene-layout-settings"\)\.addEventListener\("change"[\s\S]+scene-interface-next[\s\S]+scene-panel-layout/, "Global table settings must handle changes outside the scene workbench");
...
assert.match(companionMarkup, /id="scene-control-mode"[\s\S]+id="scene-turn-approval-mode"[\s\S]+Игроки сами[\s\S]+Через Нарратора/, "Hero Turn approval must be configurable beside the Narrator control mode");
assert.match(companionMarkup, /scene-add-enemy[\s\S]+scene-add-crowd[\s\S]+scene-add-hero/, "The add panel must prioritize enemies and Fodder while keeping hero creation secondary");
assert.match(companionMarkup, /scene-add-hero[^>]*>[\s\S]*<summary>/, "Hero creation must remain available in a collapsed secondary section");
assert.match(appSource, /base\.turnApprovalMode=scene\.turnApprovalMode==="narrator"\?"narrator":"self"/, "Shared Scene normalization must preserve the hero Turn approval policy");
...
const lwPersisted=vm.runInContext('sceneCore({rulesEdition:"lionwing",lionwing:{schema:1,started:true,choices:[{id:"choice",actorId:"hero",kind:"knockout"}],deferred:[{p:{kind:"damage",amount:2,targetId:"hero"},sourceId:"hero"}],receipts:[]},actors:[{id:"hero",kind:"hero",rulesEdition:"lionwing",space:"main",hp:16,maxHp:16,wounds:2,lionwing:{vulnerable:true,modifiers:[]}}]})',appCoreContext);
assert.equal(lwPersisted.actors[0].wounds,2,"Saving LionWing must preserve Wounds");
assert.equal(lwPersisted.actors[0].guts,null);
assert.equal(lwPersisted.actors[0].lionwing.vulnerable,true);
assert.equal(lwPersisted.lionwing.choices[0].id,"choice","A pending player decision survives reload");
assert.equal(lwPersisted.lionwing.deferred[0].p.amount,2,"Deferred repeated hits survive reload");
assert.equal(freshCampaignScene.turnApprovalMode, "self", "Fresh Scenes let players start their own hero Turns by default");
const narratorApprovalScene = vm.runInContext('sceneCore({turnApprovalMode:"narrator"})', appCoreContext);
assert.equal(narratorApprovalScene.turnApprovalMode, "narrator", "Scene persistence retains Narrator approval for hero Turns");
...
assert.match(app, /function scenePanelSide\(panel\)/, "Panel placement is resolved independently while the button rail remains fixed");
assert.match(cockpitCss, /scene-panel-left\.scene-mode:not\(\.scene-player-view\) \.scene-workbench\{\s*grid-template-columns:var\(--scene-side-panel\) minmax\(0,1fr\) 68px/, "Left panels use an embedded column without moving the right rail");
assert.match(app, /scene-table-management[\s\S]+tableRoot\.append/, "Backups and cleanup are moved out of map editing into table management");
assert.match(app, /activeSceneTool\(\)==="target"\)\{Scene\.targetIds=/, "The player participant strip can select combat targets directly");
assert.match(app, /scene-turn-strip"\)\.addEventListener\("pointerover"/, "Hovering the player participant strip mirrors token hover on the field");
assert.match(app, /\["area","wall"\]\.includes\(tool\)/, "The active terrain or wall tool cycles its type with the mouse wheel");
assert.match(app, /Math\.abs\(event\.deltaY\)<80\?4:10/, "Trackpad pinch zoom uses a finer step than a mouse wheel");
...
assert.match(app, /data-director-tab="turn"[\s\S]+data-director-tab="manual"[\s\S]+data-director-tab="roster"/, "The Narrator console separates Turn, manual adjudication, and roster work instead of stacking everything");
assert.match(app, /function directorManualStateEditor[\s\S]+ruleResourceDefinitions[\s\S]+ruleClockDefinitions/, "Manual adjudication exposes exact rule resources and technique clocks");
assert.match(app, /data-director-set-rule-clock[\s\S]+target\.ruleClocks\[clockId\]/, "Narrator clock overrides are journaled back into canonical actor state");
...
assert.match(app, /scene-turn-strip[\s\S]{0,1600}setScenePanel\(activeSceneView\(\)===\"player\"/);
...
assert.match(cockpitCss, /mode-page\[data-page="play"\]\.active\{animation:none\}/, "The heavy table view must not animate its entire DOM while entering");
assert.match(cockpitCss, /scene-rail-right[^}]+grid-column:3!important/, "The closed right rail must stay in its own zero-width grid track instead of covering the left panel");
assert.match(cockpitCss, /scene-rail:not\(:has\(\.rail-active\)\)[^{]*\{[^}]*pointer-events:none/, "An inactive rail must never intercept panel controls");
assert.match(cockpitCss, /scene-toolbar[^}]+right:calc\(var\(--scene-right-used\) \+ 68px/, "Field tools must remain anchored to the right edge of the usable board");
