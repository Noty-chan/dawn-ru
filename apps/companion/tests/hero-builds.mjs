...
assert.deepEqual(Array.from(shadowCoverage.filter(level => level.automation === "partial"), level => level.id), ["ruiner.grim-ascendant.2"], "Assassin I-II stay fully covered while the conflicting Grim Ascendant II claim remains partial");
...
assert.deepEqual(Array.from(coverageFor(upgradedShadowBuild).filter(level => level.automation === "partial"), level => level.id), ["ruiner.grim-ascendant.2"], "Upgrading Assassin keeps its family levels implemented while Grim Ascendant II remains partial");
