...
{
 const s=base();s.actors.push(enemy("collateral","enemy.modifier.collateral",5,5,{hp:7,maxHp:7,ap:0,baseAp:0,acted:true,hidden:true}));const p=E.prepareModifierConfigure(s,{actorId:"collateral",cells:["1,0","2,0","3,0"]});assert.equal(p.ok,true);const deployed=E.dispatchMany(s,p.events).scene;assert.equal(deployed.actors.filter(a=>a.profileId==="enemy.modifier.collateral").length,3);assert.ok(deployed.actors.filter(a=>a.profileId==="enemy.modifier.collateral").every(a=>!a.hidden&&a.ap===0&&a.acted),"Only deployed Collateral instances appear on the board and none takes a Turn");assert.equal(deployed.sessionClocks[0].size,2);const down=E.dispatchMany(deployed,[{id:"collateral-down",type:"actor.knockout",actorId:"carrier",payload:{targetId:"collateral"}}]).scene;assert.equal(down.sessionClocks[0].value,1);
...
{
 const s=base();s.actors.push(enemy("garg","enemy.modifier.gargantuan",5,5,{hp:0,maxHp:0,armor:1,evasion:15,ap:0,baseAp:0,acted:true,hidden:true}));assert.equal(E.turnStartStatus(s,"garg").available,false,"A modifier never receives a separate Turn");const p=E.prepareModifierConfigure(s,{actorId:"garg",carrierId:"carrier",mode:"right"});assert.equal(p.ok,true);const r=E.dispatchMany(s,p.events).scene;assert.equal(r.spaces[0].width,8);assert.equal(r.spaces[0].mode,"custom");assert.equal(r.objects.find(o=>o.metadata?.enemyModifier==="gargantuan-body")?.cells.length,7);r.actors.find(a=>a.id==="h1").x=7;r.actors.find(a=>a.id==="h1").y=3;assert.equal(E.targetStatus(r,{sourceActorId:"h1",targetIds:["carrier"],audience:"enemies",range:1,min:1,max:1}).available,true,"The added Gargantuan edge is a real range origin for attacks against its shared body");
