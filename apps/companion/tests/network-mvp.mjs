...
assert.match(syncSource, /function refreshSceneIfNewer[\s\S]+select\("version"\)[\s\S]+5000/, "a low-load version heartbeat must recover missed Realtime Scene updates without flooding a nano database");
...
assert.doesNotMatch(syncSource, /scene refresh failed[^\n]+scheduleReconnect|command refresh failed[^\n]+scheduleReconnect/, "a failed heartbeat must not create a reconnect storm");
