...
const bladeZeroStep = Engine.prepareAction(bladeMaster, data, { actorId: "hero", actionId: talentFinisher.id, attribute: "talent", targetIds: ["enemy"], roll: { attribute: "talent", rolls: [6, 5], successes: 2, crits: 0 } });
assert.equal(bladeZeroStep.ok, true, "Master III Blade accepts the canonical zero-cell path when no movement destination is selected");
assert.deepEqual(Array.from(bladeZeroStep.events.find(event => event.type === "attack.pending").payload.masterFinisher.path), [], "Blade keeps an omitted destination as an empty path");
assert.equal(Engine.dispatchMany(bladeMaster, bladeZeroStep.events).scene.pendingAction?.techniqueRuleId, "vagabond.master-at-arms.3", "Dispatch accepts the canonical zero-cell Blade path");
const bladeOneStepMaster = structuredClone(bladeMaster);
bladeOneStepMaster.actors[1].x = 3;
const bladeOneStep = Engine.prepareAction(bladeOneStepMaster, data, { actorId: "hero", actionId: talentFinisher.id, attribute: "talent", targetIds: ["enemy"], armamentDestination: { x: 2, y: 1 }, roll: { attribute: "talent", rolls: [6, 5], successes: 2, crits: 0 } });
assert.equal(bladeOneStep.ok, true, "Master III Blade accepts a canonical one-cell path");
assert.deepEqual(Array.from(bladeOneStep.events.find(event => event.type === "attack.pending").payload.masterFinisher.path), ["2,1"], "Blade records a one-cell path without adding the origin");
assert.equal(Engine.dispatchMany(bladeOneStepMaster, bladeOneStep.events).scene.pendingAction?.techniqueRuleId, "vagabond.master-at-arms.3", "Dispatch accepts the canonical one-cell Blade path");
...
const bladePending = Engine.dispatchMany(bladeMaster, bladeFinisher.events);
assert.equal(bladePending.scene.pendingAction?.techniqueRuleId, "vagabond.master-at-arms.3", "Dispatch accepts a sequential two-cell Blade path");
const bladeResolved = Engine.resolvePendingAction(bladePending.scene, data);
assert.equal(bladeResolved.ok, true, "A sequential two-cell Blade path resolves after dispatch");
const bladeFinished = Engine.dispatchMany(bladePending.scene, bladeResolved.events).scene;
assert.ok((bladeFinished.actors.find(actor => actor.id === "enemy")?.effects || []).includes("negative.помечен"), "A successful two-cell Blade path marks the crossed target after resolve");
