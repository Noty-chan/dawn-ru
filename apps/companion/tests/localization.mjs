...
for (const file of ["localization.js", "locale-ru.js", "data.js"]) vm.runInContext(fs.readFileSync(new URL(file, root), "utf8"), context, { filename: file });
...
i18n.registerLocale("en", { "scene.prompt.cancel": "Cancel" });
assert.equal(i18n.setLocale("en"), true);
assert.equal(i18n.t("scene.prompt.cancel"), "Cancel");
assert.equal(i18n.t("scene.prompt.pass"), "Не использовать", "missing English strings must fall back to Russian");
