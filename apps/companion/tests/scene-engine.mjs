...
const clockCreated = Engine.dispatch(scene, { id: "clock-create", type: "session-clock.create", payload: { id: "scene-clock-threat", name: "Приближение угрозы", size: 6 } }).scene;
assert.deepEqual(JSON.parse(JSON.stringify(clockCreated.sessionClocks[0])), { id: "scene-clock-threat", name: "Приближение угрозы", size: 6, value: 0 }, "Session clocks live in the canonical Scene state");
...
const clockRemoved = Engine.dispatch(clockRenamed, { id: "clock-remove", type: "session-clock.remove", payload: { id: "scene-clock-threat" } }).scene;
...
assert.equal(triggerRegistry.count, 12);
