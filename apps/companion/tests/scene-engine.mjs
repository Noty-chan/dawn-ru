...
assert.equal(darkUrgeStatus.count, 6, "Dark Urge +2 and Outgunned +2 compose through the dice family");
assert.deepEqual(Array.from(darkUrgeStatus.sources, source => source.ruleId), ["wolf.outgunned", "wolf.dark-urge"]);
const darkUrgePrepared = Engine.diceRollPayload(raashaDiceScene, "hero", darkUrgeRequest, { rolls: [4, 1, 1, 1, 1, 1] });
...
assert.equal(resistedScene.actors.find(actor => actor.id === "hero").stress, 1);
...
const cryoOnceScene = structuredClone(scene);
cryoOnceScene.actors[0].techniques = { "ruiner.cryomancer": 2 };
cryoOnceScene.actors[0].ruleClocks = { "ruiner.cryomancer.icicle": { clockId: "ruiner.cryomancer.icicle", label: "Сосулька", size: 4, minimumSize: 4, initial: 0, resetScope: "scene", active: true, value: 0 } };
const cryoSameAction = Engine.dispatchMany(cryoOnceScene, [
  { type: "resource.gain", actorId: "hero", payload: { resource: "focus", amount: 1, actionInstanceId: "cryo-action" } },
  { type: "resource.gain", actorId: "hero", payload: { resource: "focus", amount: 1, actionInstanceId: "cryo-action" } },
]).scene;
assert.equal(Engine.clockStatus(cryoSameAction, "hero", "ruiner.cryomancer.icicle").value, 1, "Cryomancer II fills Icicle only once per Action despite duplicate Focus gains");
const cryoNextAction = Engine.dispatchMany(cryoSameAction, [{ type: "resource.gain", actorId: "hero", payload: { resource: "focus", amount: 1, resolvedResource: "focus", resolvedDelta: 1, actionInstanceId: "cryo-next-action" } }]).scene;
assert.equal(Engine.clockStatus(cryoNextAction, "hero", "ruiner.cryomancer.icicle").value, 2, "Cryomancer II fills another Icicle segment on a later Action");
...
const icicleTargetPrompt = Engine.dispatchMany(icicleRest, icicleResponse).scene;
assert.equal(icicleTargetPrompt.pendingPrompt?.kind, "cryomancer-icicle-target", "Icicle conversion asks for its canonical target before payment");
const icicleTargetResponse = Engine.respondRulePrompt(icicleTargetPrompt, data, { choice: "target:enemy" });
assert.equal(icicleTargetResponse.ok, true, icicleTargetResponse.errors?.join(" "));
const icicleConverted = Engine.dispatchMany(icicleTargetPrompt, icicleTargetResponse.events).scene;
assert.equal(icicleConverted.pendingPrompt ?? null, null, "Icicle resolves after selecting a valid target");
assert.equal(icicleConverted.actors[0].focus, 50, "Converting Icicle replaces the Focus gained from Rest");
assert.equal(icicleConverted.actors[1].hp, 3, "Icicle deals Spirit/2 damage per emptied segment through the damage pipeline");
assert.ok(icicleConverted.actors[1].effects.includes("negative.обездвижен"), "Icicle Immobilizes a Slow target");
assert.equal(Engine.respondRulePrompt(icicleConverted, data, { choice: "target:enemy" }).ok, false, "A retried Icicle target response cannot spend Focus twice");
assert.equal(Engine.respondRulePrompt(structuredClone(icicleTargetPrompt), data, { choice: "target:enemy" }).ok, true, "An imported Icicle target prompt retains canonical provenance");
const staleIcicleTarget = structuredClone(icicleTargetPrompt);
staleIcicleTarget.actors[1].knockedOut = true;
assert.equal(Engine.respondRulePrompt(staleIcicleTarget, data, { choice: "target:enemy" }).ok, false, "Icicle rejects a target that becomes unavailable before payment");
...
assert.equal(Engine.availableActions(icicleContinued, data, "hero").find(action => action.name === "Заклинание").quick, false, "Icicle does not create stale Quick Spell quota");
assert.equal(Engine.availableActions(icicleContinued, data, "hero").find(action => action.name === "Зарядка").available, true, "Icicle resolution does not block unrelated actions");
...
const grimForgedBody = Engine.prepareAction(grimScene, data, { actorId: "hero", actionId: actionNamed("Завершение").id, attribute: "body", targetIds: ["enemy"], roll: { attribute: "body", rolls: [6, 5, 2], successes: 2, crits: 1 } });
assert.equal(grimForgedBody.ok, true, "A stale Drain Life flag cannot impose a non-canonical action restriction");
assert.equal(Engine.prepareAction(grimScene, data, { actorId: "hero", actionId: actionNamed("Завершение").id, attribute: "spirit", targetIds: ["enemy"], roll: { attribute: "spirit", rolls: [6, 5, 2], successes: 2, crits: 1 } }).events.find(event => event.type === "attack.pending")?.payload.drainLife, undefined, "Canonical Grim Ascendant does not carry the removed Drain Life payload");
...
assert.equal(grimDrainPrepared.events.find(event => event.type === "attack.pending")?.payload.damage, 4, "A stale Drain Life flag does not alter canonical Spirit Finisher damage");
...
assert.equal(grimDrainResolved.actors[0].effects.includes("positive.регенерирует"), false, "A stale Drain Life flag does not grant non-canonical Regenerating");
