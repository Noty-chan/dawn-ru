...
const duplicateArts = vm.runInContext(`sceneCore({...Scene,artworks:[{id:"same-art",image:"data:image/png;base64,AAAA"},{id:"same-art",image:"data:image/png;base64,BBBB"}]})`, app);
assert.equal(duplicateArts.artworks.length, 2, "imported artwork remains present after normalization");
assert.notEqual(duplicateArts.artworks[0].id, duplicateArts.artworks[1].id, "duplicate imported artwork IDs cannot collide in IndexedDB");
