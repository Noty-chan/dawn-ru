...
const DEFAULT_SCENE_PANEL_SIDES={director:"right",inspector:"right",sheet:"right",utility:"right",reference:"right",roster:"left",media:"right",map:"left",add:"left",table:"left",network:"right",log:"right"};
let activeDirectorTab="turn",scenePanelLayoutMode=["split","right","left","custom"].includes(store.sceneUi?.panelLayout)?store.sceneUi.panelLayout:"split",scenePanelSides={...DEFAULT_SCENE_PANEL_SIDES,...store.sceneUi?.panelSides};
function persistableStore(){
  ...
  return {...store,heroes,scene,gmLibrary:normalizeGmLibrary(store.gmLibrary),sceneUi:{zoom:sceneZoom,controlMode:sceneControlMode,panelLayout:scenePanelLayoutMode,panelSides:scenePanelSides,fitVersion:8,viewport:sceneViewportMode}};
}
...
function persist(){
  store.heroes[store.current]=S;store.scene=Scene;store.gmLibrary=normalizeGmLibrary(store.gmLibrary);store.sceneUi={zoom:sceneZoom,controlMode:sceneControlMode,panelLayout:scenePanelLayoutMode,panelSides:scenePanelSides,fitVersion:8,viewport:sceneViewportMode};scheduleHeroMediaPersistence();
  persistHeroStore();
