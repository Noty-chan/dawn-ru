...
function sceneCore(raw){
  ...
  base.actors.forEach((actor,index)=>{
    ...
    actor.ruleState=source.ruleState&&typeof source.ruleState==="object"?{pugilistStance:clamp(source.ruleState.pugilistStance,0,4),martialPerfection:Boolean(source.ruleState.martialPerfection),growth:clamp(source.ruleState.growth,0,99),imposingPresence:Boolean(source.ruleState.imposingPresence),enemyAim:clamp(source.ruleState.enemyAim,0,1),rangerHeadshotTargetId:source.ruleState.rangerHeadshotTargetId||null,berserkerLastStand:Boolean(source.ruleState.berserkerLastStand),berserkerReactionTurnSerial:clamp(source.ruleState.berserkerReactionTurnSerial,0,999999),healerGuardianId:source.ruleState.healerGuardianId||null,grimTransformed:Boolean(source.ruleState.grimTransformed),grimUsed:Boolean(source.ruleState.grimUsed),warringTransformed:Boolean(source.ruleState.warringTransformed),warringUsed:Boolean(source.ruleState.warringUsed),drainLife:Boolean(source.ruleState.drainLife),lastCreationSpellMarks:clamp(source.ruleState.lastCreationSpellMarks,0,99)}:{};
  ...
  const inSpace=(spaceId,cell)=>{const field=base.spaces.find(space=>space.id===spaceId),[x,y]=String(cell).split(",").map(Number);return Boolean(field&&Number.isInteger(x)&&Number.isInteger(y)&&x>=0&&y>=0&&x<field.width&&y<field.height)};
  base.objects=base.objects.map(object=>({...object,cells:(object.cells||[]).filter(cell=>inSpace(object.space,cell))})).filter(object=>object.cells.length);
  ...
  base.topology.cuts=base.topology.cuts.map(cut=>({...cut,cells:(cut.cells||[]).filter(cell=>inSpace(cut.space,cell))})).filter(cut=>cut.cells.length);
  ...
  if(base.activeActorId&&!actorAvailable(base.activeActorId))base.activeActorId=null;
