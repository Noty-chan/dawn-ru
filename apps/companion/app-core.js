...
function normalizeGmLibrary(raw){
  ...
  const encounters=Array.isArray(source.encounters)?source.encounters.slice(0,80).map(item=>({id:typeof item?.id==="string"?item.id:uid(),name:typeof item?.name==="string"?item.name.slice(0,120):"Столкновение",createdAt:typeof item?.createdAt==="string"?item.createdAt.slice(0,32):new Date().toISOString(),width:clamp(item?.width||7,1,12),height:clamp(item?.height||7,1,12),enemies:Array.isArray(item?.enemies)?item.enemies.slice(0,60).map(enemy=>({profileId:typeof enemy?.profileId==="string"?enemy.profileId:"",enemyVariantId:typeof enemy?.enemyVariantId==="string"?enemy.enemyVariantId:null,name:typeof enemy?.name==="string"?enemy.name.slice(0,120):"Противник",tier:clamp(enemy?.tier||1,1,99),x:clamp(enemy?.x,0,11),y:clamp(enemy?.y,0,11),hp:clamp(enemy?.hp,0,9999),maxHp:clamp(enemy?.maxHp,1,9999),ap:clamp(enemy?.ap??2,0,99),baseAp:clamp(enemy?.baseAp??2,1,99),speed:clamp(enemy?.speed,0,99),armor:clamp(enemy?.armor,0,99),evasion:clamp(enemy?.evasion,0,9999),effects:cleanArray(enemy?.effects).slice(0,30),tokenSymbol:typeof enemy?.tokenSymbol==="string"?enemy.tokenSymbol.slice(0,4):"☠",tokenColor:safeColor(enemy?.tokenColor,"#902a3d"),tokenImage:safeTokenImage(enemy?.tokenImage),portraitImage:safeImage(enemy?.portraitImage),gmRole:roles.has(enemy?.gmRole)?enemy.gmRole:"named",notes:typeof enemy?.notes==="string"?enemy.notes.slice(0,1200):""})).filter(enemy=>enemy.profileId):[]})).filter(item=>item.enemies.length):[];
...
function loadStore(){
  try{const parsed=JSON.parse(localStorage.getItem(STORAGE_KEY)||"null");if(parsed?.schema===APP_SCHEMA&&Array.isArray(parsed.heroes))return {...parsed,heroes:parsed.heroes.map(normalizeHero),scene:normalizeScene(parsed.scene),gmLibrary:normalizeGmLibrary(parsed.gmLibrary)};}catch(e){console.warn(e)}
