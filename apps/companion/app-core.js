...
function blankScene(){
  return {schema:12,version:0,name:"Структурированный бой",view:"gm",round:1,turnSerial:0,tension:0,tool:"select",activeSpace:"main",activeActorId:null,spaces:[{id:"main",name:"Основное поле",mode:"standard",width:7,height:7}],actors:[],objects:[],markers:[],topology:{cuts:[]},artworks:[],backgroundArt:null,backgroundView:{fit:"cover",position:"center",dim:28,gridOpacity:58},featuredArt:null,selectedActor:null,targetIds:[],pendingActionPlan:null,pendingAction:null,pendingPrompt:null,triggerQueue:[],challengeRequest:null,opposedRoll:null,sessionClocks:[],reminders:[],ruleHandouts:[],tools:{clocksMigrated:false},rollFeed:[],log:[],undo:[]};
}
...
function normalizeGmLibrary(raw){
  ...
  for(const encounter of encounters){const rawEncounter=(source.encounters||[]).find(item=>item?.id===encounter.id)||{};encounter.mode=["standard","cinematic","custom"].includes(rawEncounter.mode)?rawEncounter.mode:(encounter.width===7&&encounter.height===1?"cinematic":encounter.width===7&&encounter.height===7?"standard":"custom");for(let index=0;index<encounter.objects.length;index++){const rawType=rawEncounter.objects?.[index]?.type;if(["high","low","deploy-hero","deploy-enemy","objective"].includes(rawType))encounter.objects[index].type=rawType}}
