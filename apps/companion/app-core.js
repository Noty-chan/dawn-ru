...
function sceneCore(raw){
  ...
  base.actors.forEach((actor,index)=>{
    ...
    if(String(actor.profileId||"").includes(".modifier.")){const physicalImmediately=["enemy.modifier.legion","enemy.modifier.vip","lionwing.modifier.vip"].includes(actor.profileId),deployedCollateral=actor.profileId==="enemy.modifier.collateral"&&Boolean(source.modifierState?.deployed);actor.ap=0;actor.baseAp=0;actor.acted=true;actor.hidden=!(physicalImmediately||deployedCollateral)}
