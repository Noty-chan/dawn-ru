...
function normalizedEncounterApContract(enemy,{profileEdition=null,edition=null}={}){
  const profileId=String(enemy?.profileId||""),crowd=enemy?.kind==="crowd",modifier=profileId.includes(".modifier."),lionwing=edition==="lionwing"||profileEdition==="lionwing"||profileId.startsWith("lionwing."),fallback=crowd||modifier?0:lionwing?3:2;
  const number=value=>value===null||value===undefined||value===""||!Number.isFinite(Number(value))?null:clamp(value,0,99),explicitBase=number(enemy?.baseAp),explicitAp=number(enemy?.ap);
...
function normalizedSceneObjectType(object,sceneSchema,rulesEdition="ru-v0.9"){
  ...
  const supported=["attack","gas","terrain","difficult","high","low","deploy-hero","deploy-enemy","objective","danger","portal","custom"].includes(object?.type);
  return supported||(rulesEdition==="lionwing"&&object?.type==="area")?object.type:"custom";
}
...
function normalizedEffectStates(source,actor,persistedActorIds){
  ...
  const lionwing=actor.rulesEdition==="lionwing",durations=lionwing?["default","startTurn","endTurn","nextTurn","startNextOwnerTurn","endNextOwnerTurn","roundEnd","scene","persistent","manual","actionOrStartTurn"]:["default","persistent","scene","startTurn","actionOrStartTurn","roundEnd"];
  const normalizeLifetime=value=>{
    if(!value||typeof value!=="object"||Array.isArray(value))return null;
    const aliases={"start-next-owner-turn":"startNextOwnerTurn","end-next-owner-turn":"endNextOwnerTurn",nextOwnerTurnStart:"startNextOwnerTurn",nextOwnerTurnEnd:"endNextOwnerTurn",ownerTurnStart:"startNextOwnerTurn",ownerTurnEnd:"endNextOwnerTurn"},boundary=aliases[value.boundary]||value.boundary||value.kind;
    if(!["startNextOwnerTurn","endNextOwnerTurn"].includes(boundary)||typeof value.ownerActorId!=="string"||!value.ownerActorId||value.ownerActorId.length>180)return null;
    const ownerTurnSerial=Number(value.ownerTurnSerial??value.serial??value.turnSerial),sceneSerial=Number(value.sceneSerial);
    if(!Number.isSafeInteger(ownerTurnSerial)||ownerTurnSerial<0||!Number.isSafeInteger(sceneSerial)||sceneSerial<1)return null;
    return{schema:1,kind:"turn-boundary",boundary,ownerActorId:value.ownerActorId.slice(0,180),ownerTurnSerial,ownerTurnKey:`${sceneSerial}:${value.ownerActorId}:${ownerTurnSerial}`,sceneSerial,ownerTurnInstanceId:typeof value.ownerTurnInstanceId==="string"?value.ownerTurnInstanceId.slice(0,180):typeof value.turnInstanceId==="string"?value.turnInstanceId.slice(0,180):null};
  };
  const normalizeSource=item=>{
    ...
    const lifetime=normalizeLifetime(item.lifetime);if(lifetime)normalized.lifetime=lifetime;
    if(item.ownerTurnSerial!=null&&Number.isInteger(Number(item.ownerTurnSerial)))normalized.ownerTurnSerial=clamp(item.ownerTurnSerial,0,999999999);
  ...
  return Object.fromEntries(Object.entries(source.effectStates)
    ...
    .slice(0,30)
    .map(([effect,state])=>{const normalized={duration:durations.includes(state.duration)?state.duration:"default",removable:state.removable!==false,appliedTurnSerial:state.appliedTurnSerial!=null&&Number.isInteger(Number(state.appliedTurnSerial))?clamp(state.appliedTurnSerial,0,999999999):null,appliedRound:state.appliedRound!=null&&Number.isInteger(Number(state.appliedRound))?clamp(state.appliedRound,1,999):null,appliedEventId:typeof state.appliedEventId==="string"?state.appliedEventId.slice(0,120):"",sourceBound:Boolean(state.sourceBound),exclusiveBySource:Boolean(state.exclusiveBySource),sources:Array.isArray(state.sources)?state.sources.filter(item=>item&&(lionwing?(item.actorId==null||persistedActorIds.has(item.actorId)||item.sourceBound===false):persistedActorIds.has(item.actorId))).map(normalizeSource):[]};const lifetime=normalizeLifetime(state.lifetime);if(lifetime)normalized.lifetime=lifetime;return[effect,normalized]}))
}
function normalizedAuras(source, actorIds, markerIds){
  const raw=source&&typeof source==="object"?source:{};
  const values=Array.isArray(raw.auras)?raw.auras:[];
  const seen=new Set(),idPattern=/^[^\u0000-\u001f\s]{1,180}$/u,lifetimes=new Set(["default","startTurn","endTurn","nextTurn","startNextOwnerTurn","endNextOwnerTurn","roundEnd","scene","persistent","manual","actionOrStartTurn","round","chapter","session"]),relations=new Set(["ally","enemy","any"]),policies=new Set(["disable","remove"]);
  const stringValue=(value,max=180)=>typeof value==="string"&&value.trim()&&value.length<=max&&!/[\u0000-\u001f]/u.test(value)?value.trim():null;
  const integerValue=(value,min=0,max=999999999)=>Number.isSafeInteger(Number(value))&&Number(value)>=min&&Number(value)<=max?Number(value):null;
  return values.slice(0,120).map(item=>{
    if(!item||typeof item!=="object"||Array.isArray(item))return null;
    const id=stringValue(item.id),ownerActorId=stringValue(item.ownerActorId),sourceEntityId=stringValue(item.sourceEntityId),ruleId=stringValue(item.ruleId),effectId=stringValue(item.effectId),shape=item.shape&&typeof item.shape==="object"&&!Array.isArray(item.shape)?item.shape:null;
    if(!id||!idPattern.test(id)||["constructor","prototype","__proto__"].includes(id)||seen.has(id)||!ownerActorId||!sourceEntityId||!ruleId||!effectId||!shape||shape.kind!=="radius")return null;
    const distance=integerValue(shape.distance??item.distance,0,99),relation=shape.filter?.relation??item.filter?.relation??item.relation??"any";
    const rawLifetime=item.lifetime;
    let lifetime=typeof rawLifetime==="string"&&lifetimes.has(rawLifetime)?rawLifetime:null;
    if(rawLifetime&&typeof rawLifetime==="object"&&!Array.isArray(rawLifetime)){
      const boundary=rawLifetime.boundary??rawLifetime.kind??rawLifetime.phase;
      if(["startNextOwnerTurn","endNextOwnerTurn"].includes(boundary)){
        const lifetimeOwner=stringValue(rawLifetime.ownerActorId??ownerActorId),ownerTurnSerial=integerValue(rawLifetime.ownerTurnSerial??rawLifetime.serial??rawLifetime.turnSerial,0),sceneSerial=integerValue(rawLifetime.sceneSerial,1);
        if(lifetimeOwner&&ownerTurnSerial!==null&&sceneSerial!==null)lifetime={schema:1,kind:"turn-boundary",boundary,ownerActorId:lifetimeOwner,ownerTurnSerial,ownerTurnKey:`${sceneSerial}:${lifetimeOwner}:${ownerTurnSerial}`,sceneSerial,ownerTurnInstanceId:stringValue(rawLifetime.ownerTurnInstanceId??rawLifetime.turnInstanceId)||null};
      }
      else if(typeof boundary==="string"&&lifetimes.has(boundary))lifetime=boundary;
    }
    // Missing backing entities remain serialized so the engine can explain a
    // disabled aura and apply its explicit source-loss policy after reload.
    if(distance===null||!relations.has(relation)||lifetime===null)return null;
    const sourceLossPolicy=typeof item.sourceLossPolicy==="string"&&policies.has(item.sourceLossPolicy)?item.sourceLossPolicy:"disable";
    const suppressedBy=Array.isArray(item.suppressedBy)?[...new Set(item.suppressedBy.map(value=>stringValue(value)).filter(Boolean))].slice(0,12):[];
    const result={id,ownerActorId,sourceEntityId,ruleId,effectId,shape:{kind:"radius",distance},distance,filter:{relation},lifetime,removable:item.removable===true,sourceLossPolicy,suppressedBy,appliedSerial:integerValue(item.appliedSerial??0),appliedRound:integerValue(item.appliedRound??1)};
    if(stringValue(item.createdEventId))result.createdEventId=stringValue(item.createdEventId);
    if(integerValue(item.appliedChapterSerial??0)!==null)result.appliedChapterSerial=integerValue(item.appliedChapterSerial??0);
    seen.add(id);return result;
  }).filter(Boolean);
}
function normalizedTurnBoundary(value,ownerActorId,sceneSerial=1){
  if(!value||typeof value!=="object"||Array.isArray(value))return null;
  const aliases={"start-next-owner-turn":"startNextOwnerTurn","end-next-owner-turn":"endNextOwnerTurn",nextOwnerTurnStart:"startNextOwnerTurn",nextOwnerTurnEnd:"endNextOwnerTurn",ownerTurnStart:"startNextOwnerTurn",ownerTurnEnd:"endNextOwnerTurn"},boundary=aliases[value.boundary]||value.boundary||value.kind;
  if(!["startNextOwnerTurn","endNextOwnerTurn"].includes(boundary))return null;
  const owner=typeof value.ownerActorId==="string"&&value.ownerActorId?value.ownerActorId:ownerActorId,serial=Number(value.ownerTurnSerial??value.serial??value.turnSerial),scene=Number(value.sceneSerial??sceneSerial);
  if(typeof owner!=="string"||!owner||owner.length>180||!Number.isSafeInteger(serial)||serial<0||!Number.isSafeInteger(scene)||scene<1)return null;
  const instance=typeof(value.ownerTurnInstanceId??value.turnInstanceId)==="string"?(value.ownerTurnInstanceId??value.turnInstanceId).slice(0,180):null;
  return{schema:1,kind:"turn-boundary",boundary,ownerActorId:owner.slice(0,180),ownerTurnSerial:serial,ownerTurnKey:`${scene}:${owner}:${serial}`,sceneSerial:scene,ownerTurnInstanceId:instance};
}
function lionwingEntitiesApi(){
  const host=typeof window==="object"&&window?window:typeof globalThis==="object"?globalThis:null;
  return host?.DAWN_LIONWING_ENTITIES||(typeof globalThis==="object"?globalThis.DAWN_LIONWING_ENTITIES:null);
}
function normalizeLionwingEntities(base){
  if(base.rulesEdition!=="lionwing"||!base.lionwing||typeof base.lionwing!=="object"||!Object.prototype.hasOwnProperty.call(base.lionwing,"entities"))return;
  const api=lionwingEntitiesApi();
  if(!api||typeof api.reload!=="function")return;
  const hydrated=api.reload({ ...base, lionwing: base.lionwing });
  if(hydrated?.lionwing&&typeof hydrated.lionwing==="object")base.lionwing=hydrated.lionwing;
}
function stressMaximumFor(actor){
  return 3+(Array.isArray(actor?.gifts)&&actor.gifts.includes("rebel.supernatural-deafness")?1:0);
}
function spellcrafterLearnedLimitFor(level){
...
function sceneCore(raw){
  ...
  base.actors.forEach((actor,index)=>{
    ...
    if(source.kind==="crowd"){actor.kind="crowd";actor.crowdType=["mob","swarm","guards","undead","hounds","civilians","custom"].includes(source.crowdType)?source.crowdType:"mob";actor.crowdSubtype=["seeker","vortex","corpse","bodyguards-deployment","swarm-deployment"].includes(source.crowdSubtype)?source.crowdSubtype:null;actor.crowdGroupId=typeof source.crowdGroupId==="string"&&source.crowdGroupId?source.crowdGroupId.slice(0,120):actor.id;actor.seekerTargetId=actor.crowdSubtype==="seeker"&&typeof source.seekerTargetId==="string"?source.seekerTargetId.slice(0,120):null;actor.seekerOwnerId=actor.crowdSubtype==="seeker"&&typeof source.seekerOwnerId==="string"?source.seekerOwnerId.slice(0,120):null;actor.seekerDamage=actor.crowdSubtype==="seeker"?clamp(source.seekerDamage,1,9999):null;actor.vortexOwnerId=actor.crowdSubtype==="vortex"&&typeof source.vortexOwnerId==="string"?source.vortexOwnerId.slice(0,120):null;actor.summonerId=typeof source.summonerId==="string"&&persistedActorIds.has(source.summonerId)?source.summonerId:null;actor.deploymentFodderOwnerId=typeof source.deploymentFodderOwnerId==="string"&&persistedActorIds.has(source.deploymentFodderOwnerId)?source.deploymentFodderOwnerId:null;actor.source=typeof source.source==="string"?source.source.slice(0,160):"Ручное правило";actor.sourceActionId=typeof source.sourceActionId==="string"?source.sourceActionId.slice(0,180):null;actor.tier=0;actor.ap=0;actor.baseAp=0;actor.speed=0;actor.acted=true}
    if(String(actor.profileId||"").includes(".modifier.")){const physicalImmediately=["enemy.modifier.legion","enemy.modifier.vip","lionwing.modifier.vip"].includes(actor.profileId),deployedCollateral=["enemy.modifier.collateral","lionwing.modifier.collateral"].includes(actor.profileId)&&Boolean(source.modifierState?.deployed);actor.ap=0;actor.baseAp=0;actor.acted=true;actor.hidden=!(physicalImmediately||deployedCollateral)}
    ...
    actor.ruleClocks=source.ruleClocks&&typeof source.ruleClocks==="object"?Object.fromEntries(Object.entries(source.ruleClocks).filter(([id])=>/^[a-z][a-z0-9.-]{0,79}$/.test(id)).slice(0,30).map(([id,clock])=>{const raw=clock&&typeof clock==="object"?clock:{value:clock},size=clamp(Number.isSafeInteger(Number(raw.max??raw.size))?Number(raw.max??raw.size):6,1,clockLimit),minimum=clamp(Number.isSafeInteger(Number(raw.min??raw.minimum))?Number(raw.min??raw.minimum):0,0,size),initial=clamp(Number.isSafeInteger(Number(raw.initial))?Number(raw.initial):minimum,minimum,size),current=clamp(Number.isSafeInteger(Number(raw.current??raw.value))?Number(raw.current??raw.value):initial,minimum,size),lifetime=normalizedTurnBoundary(raw.lifetime,actor.id,scene.lionwing?.sceneSerial??1);return[id,{...raw,clockId:id,kind:"clock",size,max:size,maximum:size,minimumSize:minimum,min:minimum,minimum,initial,value:current,current,ownerActorId:actor.id,sourceActorId:persistedActorIds.has(raw.sourceActorId)?raw.sourceActorId:actor.id,sourceEntityId:typeof raw.sourceEntityId==="string"?raw.sourceEntityId:null,ruleId:typeof raw.ruleId==="string"?raw.ruleId.slice(0,180):null,scope:["manual","startTurn","endTurn","roundEnd","scene","turn","round","chapter","session"].includes(raw.scope)?raw.scope:(raw.resetAt||"manual"),lifetime:lifetime||(["default","startTurn","endTurn","nextTurn","startNextOwnerTurn","endNextOwnerTurn","roundEnd","scene","persistent","manual","turn","round","chapter","session"].includes(raw.lifetime)?raw.lifetime:(raw.resetAt||"manual")),active:raw.active!==false}] })):{};
    actor.ruleResources=source.ruleResources&&typeof source.ruleResources==="object"?Object.fromEntries(Object.entries(source.ruleResources).filter(([id])=>/^[a-zA-Z][\w.-]{0,79}$/.test(id)).slice(0,30).map(([id,resource])=>{const raw=resource&&typeof resource==="object"?resource:{value:resource},maximum=raw.max===null||raw.maximum===null?null:Number.isSafeInteger(Number(raw.max??raw.maximum))?clamp(raw.max??raw.maximum,0,9999):null,minimum=clamp(Number.isSafeInteger(Number(raw.min??raw.minimum))?Number(raw.min??raw.minimum):0,0,maximum??9999),initial=clamp(Number.isSafeInteger(Number(raw.initial))?Number(raw.initial):minimum,minimum,maximum??9999),current=clamp(Number.isSafeInteger(Number(raw.current??raw.value))?Number(raw.current??raw.value):initial,minimum,maximum??9999),threshold=raw.threshold==null?maximum:clamp(raw.threshold,minimum,maximum??9999),lifetime=normalizedTurnBoundary(raw.lifetime,actor.id,scene.lionwing?.sceneSerial??1);return[id,{...raw,resource:id,kind:"resource",label:typeof raw.label==="string"?raw.label.slice(0,120):id,max:maximum,maximum,min:minimum,minimum,current,value:current,initial,threshold,ownerActorId:actor.id,sourceActorId:persistedActorIds.has(raw.sourceActorId)?raw.sourceActorId:actor.id,sourceEntityId:typeof raw.sourceEntityId==="string"?raw.sourceEntityId:null,ruleId:typeof raw.ruleId==="string"?raw.ruleId.slice(0,180):null,scope:["manual","startTurn","endTurn","roundEnd","scene","turn","round","chapter","session"].includes(raw.scope)?raw.scope:(raw.resetAt||"manual"),lifetime:lifetime||(["default","startTurn","endTurn","startNextOwnerTurn","endNextOwnerTurn","roundEnd","scene","persistent","manual","nextTurn","turn","round","chapter","session"].includes(raw.lifetime)?raw.lifetime:(raw.resetAt||"manual")),replaces:raw.replaces=== "focus"?"focus":null,replacesAp:raw.replacesAp===true,inverted:raw.inverted===true}] })):{};
    ...
    actor.ruleState=source.ruleState&&typeof source.ruleState==="object"?{pugilistStance:clamp(source.ruleState.pugilistStance,0,4),martialPerfection:Boolean(source.ruleState.martialPerfection),growth:clamp(source.ruleState.growth,0,99),gluttonConsumed:clamp(source.ruleState.gluttonConsumed,0,99),imposingPresence:Boolean(source.ruleState.imposingPresence),enemyAim:clamp(source.ruleState.enemyAim,0,1),rangerHeadshotTargetId:source.ruleState.rangerHeadshotTargetId||null,berserkerLastStand:Boolean(source.ruleState.berserkerLastStand),berserkerReactionTurnSerial:clamp(source.ruleState.berserkerReactionTurnSerial,0,999999),healerGuardianId:source.ruleState.healerGuardianId||null,privateerGearChange:Boolean(source.ruleState.privateerGearChange),roninSheathed:Boolean(source.ruleState.roninSheathed),grimTransformed:Boolean(source.ruleState.grimTransformed),grimUsed:Boolean(source.ruleState.grimUsed),warringTransformed:Boolean(source.ruleState.warringTransformed),warringUsed:Boolean(source.ruleState.warringUsed),lastCreationSpellMarks:clamp(source.ruleState.lastCreationSpellMarks,0,99),bodyguardsBrace:source.ruleState.bodyguardsBrace&&Array.isArray(source.ruleState.bodyguardsBrace.zoneIds)?{zoneIds:[...new Set(source.ruleState.bodyguardsBrace.zoneIds.filter(id=>typeof id==="string"&&persistedActorIds.has(id)))].slice(0,20)}:null}:{};
    actor.deploymentProxy=source.deploymentProxy===true&&["lionwing.npc.bodyguards","lionwing.npc.swarm"].includes(actor.profileId);
    actor.deploymentFodderIds=actor.deploymentProxy?cleanArray(source.deploymentFodderIds).filter(id=>persistedActorIds.has(id)).slice(0,100):[];
    ...
    // Keep only the typed, scene-owned mode state. Conditions and receipts are
    // re-derived from the journal; imported UI fields cannot grant a mode.
    const rawModes=source.ruleModes&&typeof source.ruleModes==="object"?source.ruleModes:{};
    actor.ruleModes=Object.fromEntries(Object.entries(rawModes).filter(([group,value])=>group==="vagabond.master-at-arms.armament"&&value&&typeof value==="object"&&["blade","polearm","chain"].includes(value.modeId)).slice(0,8).map(([group,value])=>[group,{schema:1,modeId:value.modeId,label:typeof value.label==="string"?value.label.slice(0,40):value.modeId,ruleId:"vagabond.master-at-arms.1",sourceRuleId:"vagabond.master-at-arms.1",sourceDigest:"d35f468065e84fbb0c86bc60015632bdbcfe9b0ced2ed2cfa370453f64a72371",actionId:"action.атаки.стычка",stateScope:"scene",equippedTurnSerial:clamp(value.equippedTurnSerial,0,999999999),receipt:value.receipt&&typeof value.receipt==="object"?{schema:1,key:typeof value.receipt.key==="string"?value.receipt.key.slice(0,180):"",turnSerial:clamp(value.receipt.turnSerial,0,999999999),useIndex:clamp(value.receipt.useIndex,1,99)}:null}]));
    const modifierProfile=String(actor.profileId||"").includes(".modifier."),rawModifier=source.modifierState&&typeof source.modifierState==="object"?source.modifierState:{};
actor.modifierState=modifierProfile?{carrierId:typeof rawModifier.carrierId==="string"?rawModifier.carrierId:null,targetId:typeof rawModifier.targetId==="string"?rawModifier.targetId:null,cells:cleanArray(rawModifier.cells).slice(0,64),mode:["inward","outward","lines","square3","rect2x5","edges","left","right","top","bottom","burn","freeze","accelerate","toughen"].includes(rawModifier.mode)?rawModifier.mode:null,configuredRound:clamp(rawModifier.configuredRound,0,999),lastActionRound:clamp(rawModifier.lastActionRound,0,999),lastFlipBoundaryEventId:typeof rawModifier.lastFlipBoundaryEventId==="string"?rawModifier.lastFlipBoundaryEventId:null,lastFollowTurnSerial:rawModifier.lastFollowTurnSerial==null?null:clamp(rawModifier.lastFollowTurnSerial,0,999999999),groupId:typeof rawModifier.groupId==="string"?rawModifier.groupId.slice(0,120):null,clockId:typeof rawModifier.clockId==="string"?rawModifier.clockId.slice(0,120):null,deployed:Boolean(rawModifier.deployed),absorbed:clamp(rawModifier.absorbed,0,999),expanded:Boolean(rawModifier.expanded),legionHp:clamp(rawModifier.legionHp,0,9999),instanceRootId:typeof rawModifier.instanceRootId==="string"?rawModifier.instanceRootId.slice(0,120):null}:{};
  ...
  if(base.rulesEdition==="lionwing"&&Array.isArray(scene.areas))base.areas=scene.areas.slice(0,240).map(area=>({id:typeof area.id==="string"?area.id:uid(),space:spaceIds.has(area.space)?area.space:base.activeSpace,type:"area",label:typeof area.label==="string"?area.label.slice(0,80):"Область",source:typeof area.source==="string"?area.source.slice(0,160):"Ручное правило",ruleId:typeof area.ruleId==="string"?area.ruleId.slice(0,180):"",duration:area.duration==="turn"?"endTurn":["instant","endTurn","nextTurn","round","scene","persistent"].includes(area.duration)?area.duration:"scene",ownerActorId:actorIds.has(area.ownerActorId)?area.ownerActorId:null,cells:cleanArray(area.cells).slice(0,144),createdRound:clamp(area.createdRound||1,1,999),hidden:Boolean(area.hidden),visibility:["public","owner","hidden"].includes(area.visibility)?area.visibility:null,metadata:area.metadata&&typeof area.metadata==="object"?area.metadata:{}})).filter(area=>area.id);
  ...
  base.markers=Array.isArray(scene.markers)?scene.markers.slice(0,240).map(marker=>({id:typeof marker.id==="string"?marker.id:uid(),space:spaceIds.has(marker.space)?marker.space:base.activeSpace,x:clamp(marker.x,0,11),y:clamp(marker.y,0,11),kind:["mark","damocles","bomb","ritual","trap","summon","weapon","objective","countdown","hidden","custom","corpse"].includes(marker.kind)?marker.kind:"mark",label:typeof marker.label==="string"?marker.label.slice(0,80):"Метка",color:safeColor(marker.color,"#e2b54a"),source:typeof marker.source==="string"?marker.source.slice(0,160):"Ручное правило",ruleId:typeof marker.ruleId==="string"?marker.ruleId.slice(0,180):"",duration:["endTurn","nextTurn","round","scene","persistent"].includes(marker.duration)?marker.duration:"scene",ownerActorId:actorIds.has(marker.ownerActorId)?marker.ownerActorId:null,sourceActorId:actorIds.has(marker.sourceActorId)?marker.sourceActorId:null,sourceLossPolicy:["detach","remove"].includes(marker.sourceLossPolicy)?marker.sourceLossPolicy:null,hostActorId:actorIds.has(marker.hostActorId)?marker.hostActorId:null,offset:marker.offset&&Number.isInteger(Number(marker.offset.dx))&&Number.isInteger(Number(marker.offset.dy))?{dx:Number(marker.offset.dx),dy:Number(marker.offset.dy)}:null,createdRound:clamp(marker.createdRound||1,1,999),metadata:marker.metadata&&typeof marker.metadata==="object"?marker.metadata:{}})):[];
  const rawAuras=Array.isArray(base.lionwing?.auras)?base.lionwing.auras:Array.isArray(scene.auras)?scene.auras:[];
  if(base.rulesEdition==="lionwing"||rawAuras.length){base.lionwing||={};base.lionwing.auras=normalizedAuras({auras:rawAuras},actorIds,new Set(base.markers.map(marker=>marker.id)));}
  base.objects.forEach((object,index)=>{const source=scene.objects?.[index]||{};object.labelI18n=sceneI18n(source.labelI18n,80);object.sourceI18n=sceneI18n(source.sourceI18n,160)});
  base.walls.forEach((wall,index)=>{const source=scene.walls?.[index]||{};wall.labelI18n=sceneI18n(source.labelI18n,80);wall.sourceI18n=sceneI18n(source.sourceI18n,160)});
  base.markers.forEach((marker,index)=>{const source=scene.markers?.[index]||{};marker.labelI18n=sceneI18n(source.labelI18n,80);marker.sourceI18n=sceneI18n(source.sourceI18n,160)});
...
function addEnemyDeploymentPassives(scene, actor){
  const prepared=SceneEngine.prepareEnemyDeployment(scene,actor);if(!prepared.ok)throw new Error(prepared.errors.join(" ")||"Не удалось подготовить особые Зоны массовки.");
  if(prepared.owner&&actor)Object.assign(actor,prepared.owner);
  scene.actors.push(...prepared.zones);return prepared.zones;
}
...
const TECHNIQUE_STATUS_LABEL_KEYS=Object.freeze({full:"builder.techniques.statusFull",decision:"builder.techniques.statusDecision",partial:"builder.techniques.statusPartial",manual:"builder.techniques.statusManual"});
const TECHNIQUE_STATUS_SHORT_KEYS=Object.freeze({full:"builder.techniques.statusFullShort",decision:"builder.techniques.statusDecisionShort",partial:"builder.techniques.statusPartialShort",manual:"builder.techniques.statusManualShort"});
const techniqueStatusIcon=status=>status==="partial"?"◐":status==="manual"?"—":"⚙";
...
function techniqueAutomationStatusMarkup(status,{short=false}={}){const normalized=LIONWING_AUTOMATION_STATUSES.has(status)?status:"manual",label=techniqueAutomationStatusLabel(normalized,{short}),fullLabel=techniqueAutomationStatusLabel(normalized);return`<span class="technique-status technique-status-${normalized}" title="${esc(fullLabel)}" aria-label="${esc(fullLabel)}"><span aria-hidden="true">${techniqueStatusIcon(normalized)}</span><span>${esc(label)}</span></span>`}
function techniqueLevelStatusReason(status){if(status?.reason)return status.reason;if(status?.automation==="partial")return t("builder.techniques.statusPartialHelp");if(status?.automation==="manual")return t("builder.techniques.statusManualHelp");return""}
function techniqueMatchesStatus(technique,status="all"){if(status==="all")return true;if(!LIONWING_AUTOMATION_STATUSES.has(status))return false;return(technique?.levels||[]).some(level=>techniqueLevelAutomation(technique.id,level.n)===status)}
function techniqueStatusSummaryMarkup(technique,levels=technique?.levels||[]){const counts=new Map();for(const level of levels){const status=techniqueLevelAutomation(technique.id,level.n);counts.set(status,(counts.get(status)||0)+1)}const label=t("builder.techniques.statusSummary"),items=["full","decision","partial","manual"].filter(status=>counts.has(status)).map(status=>`<span class="technique-status-count technique-status-${status}"><b>${counts.get(status)}</b> ${esc(techniqueAutomationStatusLabel(status,{short:true}))}</span>`);return`<span class="technique-status-summary" aria-label="${esc(label)}"><span class="technique-status-summary-label">${esc(label)}:</span> ${items.join(" · ")}</span>`}
function enemyProfileAutomation(profile){if(["lionwing.modifier.isolation","lionwing.modifier.artillery","lionwing.modifier.haven","lionwing.modifier.contagion","lionwing.modifier.earthquake","lionwing.modifier.vip","lionwing.modifier.collateral","lionwing.modifier.legion","lionwing.modifier.vortex","lionwing.modifier.blaze","lionwing.modifier.giant","lionwing.modifier.gargantuan"].includes(profile?.id))return"partial";const statuses=(profile?.rules||[]).map(rule=>SceneEngine?.enemyRuleAutomation?.(rule.id)||"assisted"),automated=statuses.filter(status=>status!=="assisted").length;return automated===statuses.length&&automated?"full":automated?"partial":"assisted"}
...
function sceneCombatStarted(scene=Scene){return Boolean(scene.activeActorId||Number(scene.round||1)>1||(scene.actors||[]).some(actor=>actor.kind!=="crowd"&&!String(actor.profileId||"").includes(".modifier.")&&actor.acted))}
