...
function normalizedEncounterApContract(enemy,{profileEdition=null,edition=null}={}){
  const profileId=String(enemy?.profileId||""),crowd=enemy?.kind==="crowd",modifier=profileId.startsWith("enemy.modifier."),lionwing=edition==="lionwing"||profileEdition==="lionwing"||profileId.startsWith("lionwing."),fallback=crowd||modifier?0:lionwing?3:2;
...
function sceneCore(raw){
  ...
  base.actors.forEach((actor,index)=>{
    ...
    actor.ruleState=source.ruleState&&typeof source.ruleState==="object"?{pugilistStance:clamp(source.ruleState.pugilistStance,0,4),martialPerfection:Boolean(source.ruleState.martialPerfection),cocoonAwake:Boolean(source.ruleState.cocoonAwake),growth:clamp(source.ruleState.growth,0,99),gluttonConsumed:clamp(source.ruleState.gluttonConsumed,0,99),imposingPresence:Boolean(source.ruleState.imposingPresence),enemyAim:clamp(source.ruleState.enemyAim,0,1),rangerHeadshotTargetId:source.ruleState.rangerHeadshotTargetId||null,berserkerLastStand:Boolean(source.ruleState.berserkerLastStand),berserkerReactionTurnSerial:clamp(source.ruleState.berserkerReactionTurnSerial,0,999999),healerGuardianId:source.ruleState.healerGuardianId||null,privateerGearChange:Boolean(source.ruleState.privateerGearChange),roninSheathed:Boolean(source.ruleState.roninSheathed),grimTransformed:Boolean(source.ruleState.grimTransformed),grimUsed:Boolean(source.ruleState.grimUsed),warringTransformed:Boolean(source.ruleState.warringTransformed),warringUsed:Boolean(source.ruleState.warringUsed),lastCreationSpellMarks:clamp(source.ruleState.lastCreationSpellMarks,0,99)}:{};
...
function budgets(){
  ...
  // Raasha's three Psionic disciplines are her Ability, stored as Skills only because
  // their independent ranks cannot be represented by the regular Ability builder.
