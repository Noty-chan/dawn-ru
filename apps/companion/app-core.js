...
function blankHero(){
  return {
    ...
    techniques:{},mods:{taintedBody:false,gadgetSpent:0,performanceSkill:null,spellcrafterAugments:[]},
...
function sceneCore(raw){
  ...
  base.actors.forEach((actor,index)=>{
    ...
    const spellcrafterLevel=Number(actor.techniques?.["ruiner.spellcrafter"]||0),spellcrafterIds=new Set(["fierce","focused","wild","outstanding"]),learned=[...new Set(cleanArray(source.techniqueState?.spellcrafterLearnedModifiers).filter(value=>spellcrafterIds.has(value)))].slice(0,spellcrafterLevel);
    actor.techniqueState={cunningPlan:clamp(source.techniqueState?.cunningPlan,0,4),studiedActorIds:cleanArray(source.techniqueState?.studiedActorIds).slice(0,120),spellcrafterLearnedModifiers:learned,spellModifiers:[...new Set(cleanArray(source.techniqueState?.spellModifiers).filter(value=>learned.includes(value)))].slice(0,spellcrafterLevel>=3?2:1)};
