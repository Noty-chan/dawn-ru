...
function sceneCore(raw){
  ...
  base.actors.forEach((actor,index)=>{
    ...
    if(source.kind==="crowd"){actor.kind="crowd";actor.crowdType=["mob","swarm","guards","undead","hounds","civilians","custom"].includes(source.crowdType)?source.crowdType:"mob";actor.crowdSubtype=source.crowdSubtype==="seeker"?"seeker":null;actor.crowdGroupId=typeof source.crowdGroupId==="string"&&source.crowdGroupId?source.crowdGroupId.slice(0,120):actor.id;actor.seekerTargetId=actor.crowdSubtype==="seeker"&&typeof source.seekerTargetId==="string"?source.seekerTargetId.slice(0,120):null;actor.seekerOwnerId=actor.crowdSubtype==="seeker"&&typeof source.seekerOwnerId==="string"?source.seekerOwnerId.slice(0,120):null;actor.seekerDamage=actor.crowdSubtype==="seeker"?clamp(source.seekerDamage,1,9999):null;actor.source=typeof source.source==="string"?source.source.slice(0,160):"Ручное правило";actor.tier=0;actor.ap=0;actor.baseAp=0;actor.speed=0;actor.acted=true}
