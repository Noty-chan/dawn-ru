...
function blankScene(){
  return {schema:7,version:0,name:"Структурированный бой",view:"gm",round:1,tension:0,tool:"select",activeSpace:"main",activeActorId:null,spaces:[{id:"main",name:"Основное поле",width:7,height:7}],actors:[],objects:[],markers:[],artworks:[],backgroundArt:null,backgroundView:{fit:"cover",position:"center",dim:28,gridOpacity:58},featuredArt:null,selectedActor:null,targetIds:[],pendingAction:null,pendingPrompt:null,rollFeed:[],log:[],undo:[]};
}
...
function sceneCore(raw){
  ...
  base.actors.forEach((actor,index)=>{
    ...
    actor.ruleState=source.ruleState&&typeof source.ruleState==="object"?{pugilistStance:clamp(source.ruleState.pugilistStance,0,4),martialPerfection:Boolean(source.ruleState.martialPerfection),growth:clamp(source.ruleState.growth,0,99),imposingPresence:Boolean(source.ruleState.imposingPresence),grimTransformed:Boolean(source.ruleState.grimTransformed),grimUsed:Boolean(source.ruleState.grimUsed),drainLife:Boolean(source.ruleState.drainLife),lastCreationSpellMarks:clamp(source.ruleState.lastCreationSpellMarks,0,99)}:{};
  ...
  const targetableActorIds=new Set(base.actors.filter(actor=>!actor.knockedOut).map(actor=>actor.id));base.selectedActor=actorIds.has(scene.selectedActor)?scene.selectedActor:null;base.activeActorId=actorIds.has(scene.activeActorId)?scene.activeActorId:null;base.targetIds=cleanArray(scene.targetIds).filter(id=>targetableActorIds.has(id)).slice(0,40);base.pendingAction=scene.pendingAction&&typeof scene.pendingAction==="object"?scene.pendingAction:null;base.pendingPrompt=scene.pendingPrompt&&typeof scene.pendingPrompt==="object"?scene.pendingPrompt:null;base.rollFeed=Array.isArray(scene.rollFeed)?scene.rollFeed.slice(0,20):[];
