...
function sceneCore(raw){
  ...
  base.actors.forEach((actor,index)=>{
    ...
    actor.compoundId=actor.team==="enemy"&&typeof source.compoundId==="string"&&source.compoundId.trim()?source.compoundId.trim().slice(0,120):null;
  ...
  const compoundGroups=new Map();for(const actor of base.actors.filter(item=>item.compoundId)){if(!compoundGroups.has(actor.compoundId))compoundGroups.set(actor.compoundId,[]);compoundGroups.get(actor.compoundId).push(actor)}for(const parts of compoundGroups.values()){if(parts.length<2)continue;const anchor=parts[0],effects=[...new Set(parts.flatMap(part=>part.effects||[]))],effectStates=Object.assign({},...parts.map(part=>part.effectStates||{})),alive=parts.some(part=>Number(part.hp||0)>0);parts.forEach(part=>{part.space=anchor.space;part.x=anchor.x;part.y=anchor.y;part.compoundBaseSpeed=part.compoundBaseSpeed??part.speed;part.effects=[...effects];part.effectStates=Object.fromEntries(effects.filter(effect=>effectStates[effect]).map(effect=>[effect,effectStates[effect]]));part.knockedOut=!alive})}
