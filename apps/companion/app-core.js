...
function sceneCore(raw){
  ...
  const targetableActorIds=new Set(base.actors.filter(actor=>!actor.knockedOut).map(actor=>actor.id));base.selectedActor=actorIds.has(scene.selectedActor)?scene.selectedActor:null;base.activeActorId=actorIds.has(scene.activeActorId)?scene.activeActorId:null;base.targetIds=cleanArray(scene.targetIds).filter(id=>targetableActorIds.has(id)).slice(0,40);base.pendingAction=scene.pendingAction&&typeof scene.pendingAction==="object"?scene.pendingAction:null;base.pendingPrompt=scene.pendingPrompt&&typeof scene.pendingPrompt==="object"?scene.pendingPrompt:null;base.triggerQueue=Array.isArray(scene.triggerQueue)?scene.triggerQueue.filter(item=>item&&typeof item.key==="string"&&item.event?.type==="rule.prompt").slice(0,24):[];base.rollFeed=Array.isArray(scene.rollFeed)?scene.rollFeed.slice(0,20):[];
