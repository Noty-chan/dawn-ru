...
(function (global) {
  ...
  const eventTrigger = ({ id, label, sourceDigest, coverage = "full", triggerKey, operations = [], choices = [], boundaryOperations }) => ({ id, techniqueId: id.replace(/\.\d+$/, ""), level: Number(id.match(/\.(\d+)$/)?.[1] || 0), label, sourceDigest, coverage, available: actor => knows(actor, id.replace(/\.\d+$/, ""), Number(id.match(/\.(\d+)$/)?.[1] || 0)), triggerKey, operations, choices, boundaryOperations });
