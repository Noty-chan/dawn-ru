...
Статус результата: **foundation**. Это карта канона и существующих production-маршрутов, а не повышение готовности игровых правил. Runtime не изменён.
...
Фактический `enemyRuleAutomation` на этой базе возвращает `34 attack`, `11 full`, `2 state`, `75 assisted`. Это только выбор production-ветки. `full`, `state` и `attack` не означают полного покрытия текста, а `assisted` не означает отсутствия уже существующего вспомогательного кода. Все 41 пассива намеренно имеют статус `unknown`, пока не доказан их trigger → writer → persistence маршрут по каждой фразе.
