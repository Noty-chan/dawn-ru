...
Статус: **verified** на кодовом SHA `06d9f1a`. Storage, normalizer, production
writer и точный браузерный сценарий восстановления проверены.
...
- после восстановления в exploratory-сцене состояние checkpoint оказалось уже
  ...
  восстановить и удалить изменение» первоначально не был засчитан.

После интеграции выполнен отдельный точный сценарий на текущем коде: создана
ручная точка, через UI добавлен свободный токен `Временный токен R10`, применена
точка с подтверждением, токен исчез, кнопка «Отменить» стала доступна, а после
reload в persisted Scene остался только исходный герой. Console: 0 errors и
0 warnings.
...
R10 verified для создания точки, наблюдаемого отката и reload. File chooser
import и искусственное исчерпание quota остаются дополнительными проверками,
они не блокируют этот узкий recovery-контракт.
