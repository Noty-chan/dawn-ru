...
| `turn-lifecycle` · Жизненный цикл Хода и Раунда | `lionwing-adapters.js / lionwing-engine.js` | Готовая общая возможность; отдельный уровень всё равно должен явно зарегистрировать адаптер. |
| `scene-lifecycle` · Начало, конец и сброс Сцены | `lionwing-adapters.js / lionwing-engine.js` | Готовая общая возможность; отдельный уровень всё равно должен явно зарегистрировать адаптер. |
